...
import { getOptimizedFFmpegParams, showOptimizationInfo } from "./ffmpeg-optimization.mjs";
...
async function downloadVideo(url, inputDir) {
  ...
  const existingFile = existingFiles.find((file) => file.includes(urlHash) && !file.includes("_clipped"));

  let downloadedPath;
  
  if (existingFile) {
    downloadedPath = path.join(inputDir, existingFile);
    console.log(`发现已下载的视频: ${downloadedPath}`);
  } else {
    // 下载新视频
    const ts = Date.now();
    const fileName = `${ts}_${urlHash}.mp4`;
    downloadedPath = path.join(inputDir, fileName);

    console.log(`正在下载视频到: ${downloadedPath}`);
    execCommand(
      `curl -L --fail --retry 3 --retry-delay 1 -o "${downloadedPath}" "${actualUrl}"`
    );
  }
  ...
  if (clipInfo) {
    ...
    console.log(`✂️  正在裁剪视频: ${clipInfo.start}s - ${clipInfo.end}s (时长: ${duration}s)`);
 ...
 */
async function replaceVideoSegments(mergedVideoPath, videoReplaceUrls, outputDir, inputDir) {
  ...
  // 按时间范围排序替换项
  const sortedReplacements = [...videoReplaceUrls].sort((a, b) => a.timeRange[0] - b.timeRange[0]);
  ...
  for (const replacement of sortedReplacements) {
    ...
    if (currentTime < startTime) {
      ...
      console.log(`   📹 保留原视频: ${currentTime.toFixed(2)}s - ${startTime.toFixed(2)}s`);
    }
    
    // 下载并裁剪替换视频
    console.log(`   🔄 替换片段: ${startTime.toFixed(2)}s - ${actualEndTime.toFixed(2)}s`);
    const replacementDuration = actualEndTime - startTime;
    
    // 下载替换视频
    const replacementVideoPath = await downloadVideo(replacement.url, inputDir);
  ...
  if (currentTime < totalDuration) {
    ...
    console.log(`   📹 保留原视频: ${currentTime.toFixed(2)}s - ${totalDuration.toFixed(2)}s`);
  }
  ...
  // 合并所有片段
  console.log(`\n   🔗 合并所有片段...`);
  const concatListPath = path.join(outputDir, `concat_replace_${Date.now()}.txt`);
...
async function convertToAspectRatio(videoPath, targetAspect, outputDir) {
  ...
  console.log(`🔄 转换视频比例: ${currentWidth}x${currentHeight} (${(currentAspect).toFixed(2)}) -> ${targetAspect} (${targetAspectRatio.toFixed(2)})`);
  ...
  const outputPath = path.join(outputDir, `${base}_${targetAspect.replace(':', 'x')}.mp4`);
...
export default async function runMergeVideo(config) {
  ...
  const { urls, switch: transitionEffect, transitions, videoReplaceUrls } = config;
  ...
  try {
    ...
    for (let i = 0; i < urls.length; i++) {
      ...
      if (url.startsWith("https://") || url.startsWith("http://") || url.startsWith("CLIP:")) {
    ...
    for (let i = 0; i < videoPaths.length; i++) {
      ...
      const convertedPath = await convertToAspectRatio(videoPaths[i], targetAspect, outputDir);
    ...
    if (videoReplaceUrls && Array.isArray(videoReplaceUrls) && videoReplaceUrls.length > 0) {
      console.log(`\n🔄 开始处理视频片段替换...`);
      finalVideoPath = await replaceVideoSegments(mergedVideoPath, videoReplaceUrls, outputDir, inputDir);
