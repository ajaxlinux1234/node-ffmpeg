      ...
      if (transitionFilter) {
        if (allHaveAudio) {
          ...
          if (video1HasAudio && !video2HasAudio) {
            ...
            filterComplex = `-filter_complex "[0:v][1:v]${transitionFilter}[v]" -map "[v]" -map "0:a"`;
          } else if (!video1HasAudio && video2HasAudio) {
            // 第一个没音频，第二个有：只保留第二个的音频
            filterComplex = `-filter_complex "[0:v][1:v]${transitionFilter}[v]" -map "[v]" -map "1:a"`;
          } else {
            // 都没有音频
        ...
        if (allHaveAudio) {
          ...
          if (video1HasAudio && !video2HasAudio) {
            // 第一个有音频，第二个没有：只保留第一个的音频
            filterComplex = `-filter_complex "[0:v][1:v]concat=n=2:v=1:a=0[v]" -map "[v]" -map "0:a"`;
          } else if (!video1HasAudio && video2HasAudio) {
            // 第一个没音频，第二个有：只保留第二个的音频
            filterComplex = `-filter_complex "[0:v][1:v]concat=n=2:v=1:a=0[v]" -map "[v]" -map "1:a"`;
          } else {
            // 都没有音频
