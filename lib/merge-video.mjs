      ...
      if (someHaveAudio) {
        ...
        if (audioInputs.length > 0) {
          if (audioInputs.length === 1) {
            ...
            execCommand(`ffmpeg -y ${inputs} -filter_complex "${filterComplex}" -map "[v]" -map "${audioInputs[0].replace('[', '').replace(']', '')}" -c:v ${CONFIG_VIDEO.VIDEO_CODEC} -pix_fmt ${CONFIG_VIDEO.PIXEL_FORMAT} -crf ${CONFIG_VIDEO.CRF_VALUE} -preset ${CONFIG_VIDEO.PRESET_MEDIUM} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${outputPath}"`);
          } else {
            // 多个音频流，进行拼接
            filterComplex += audioInputs.join('') + `concat=n=${audioInputs.length}:v=0:a=1[a];`;
            execCommand(`ffmpeg -y ${inputs} -filter_complex "${filterComplex}" -map "[v]" -map "[a]" -c:v ${CONFIG_VIDEO.VIDEO_CODEC} -pix_fmt ${CONFIG_VIDEO.PIXEL_FORMAT} -crf ${CONFIG_VIDEO.CRF_VALUE} -preset ${CONFIG_VIDEO.PRESET_MEDIUM} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${outputPath}"`);
          }
        } else {
          execCommand(`ffmpeg -y ${inputs} -filter_complex "${filterComplex}" -map "[v]" -c:v ${CONFIG_VIDEO.VIDEO_CODEC} -pix_fmt ${CONFIG_VIDEO.PIXEL_FORMAT} -crf ${CONFIG_VIDEO.CRF_VALUE} -preset ${CONFIG_VIDEO.PRESET_MEDIUM} "${outputPath}"`);
        }
      } else {
        execCommand(`ffmpeg -y ${inputs} -filter_complex "${filterComplex}" -map "[v]" -c:v ${CONFIG_VIDEO.VIDEO_CODEC} -pix_fmt ${CONFIG_VIDEO.PIXEL_FORMAT} -crf ${CONFIG_VIDEO.CRF_VALUE} -preset ${CONFIG_VIDEO.PRESET_MEDIUM} "${outputPath}"`);
      }
...
export default async function runMergeVideo(config) {
  ...
  // 显示当前转场效果的应用场景
  const sceneDescription = getTransitionSceneDescription(effectName);
  console.log(`应用场景: ${sceneDescription}`);
