...
export const history_deepseek_result_txt_fn = () => {
  ...
  // 图一人物距离镜头40厘米, 几乎紧贴镜头, 图二人物在图一人物身后15米, 两人在一条直线上, 两人的身高是一致的, 场景为图一的场景
  ...
  return title.map((one, index) => {
    return {
      title: one,
      // 散发古典氛围
      prompt: `生成一张比例为9:16的写实风格图片，画面质感丰富。 以平视且略俯视角，确保构图完整，着重刻画其情感。画面主色调为棕色、红色与暗黄色，营造沉稳庄重之感，${originTitle[index]},${prompt[index]}`,
      shot: customShot,
      lines‌: lines‌[index],
    };
  });
