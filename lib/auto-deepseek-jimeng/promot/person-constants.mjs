...
export const history_deepseek_result_txt_fn = () => {
  ...
  // 运镜方式：镜头跟随图中主要人物，第一秒人物停顿下来做当前场景的事情，后五秒自然走到下一场景，图中场景缓慢自然变成下一场景伴随着图中人物从当前场景缓慢变成下一个场景的人物，人物不要走动，人物只进行形态和样貌上的变化，场景变成下一场景，同时人物变换为下一场景的主要人物，动态转换流畅自然

  // 图一人物距离镜头40厘米, 几乎紧贴镜头, 图二人物在图一人物身后5米, 两人在一条直线上, 两人的身高是一致的,后面的人物要显示出来, 场景为图一的场景
  // 镜头不要动, 前面的人物自然优雅向镜头走来, 后面的人物也自然优雅紧跟着走来, 两人在一条直线上
  // 场景变成下一场景, 前面的人物从镜头自然优雅从左侧开显示, 后面的人物也自然优雅紧跟着自然向镜头前走来, 与前面的人物在一条线上上走, 后面的人物跟下一场景的人物是同一人物
  // 镜头不要动, 图中人物自然优雅地往前走
  ...
  return title.map((one, index) => {
    return {
      title: one,
      // 散发古典氛围, 画面主色调为棕色、红色与暗黄色，营造沉稳庄重之感
      prompt: `生成一张比例为9:16的写实风格图片，画面质感丰富。 以平视且略俯视角，确保构图完整，着重刻画其情感。${originTitle[index]},${prompt[index]}`,
      shot: customShot,
      lines‌: lines‌[index],
    };
  });
