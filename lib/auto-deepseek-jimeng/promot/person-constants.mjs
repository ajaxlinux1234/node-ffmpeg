...
export const history_deepseek_result_txt_fn = () => {
  ...
  const originTitle = takeRight(
    Array.from(document.querySelectorAll('body tr'))
      .map(one => 
       one.innerText.slice(2)
        ?.replace(/\s\/\s/,'/')
        .replace(/^\t/,'')
        .replace(/\t/g,'\n')
      ),
  ...
  const customShot = "运镜方式：镜头跟随图中主要人物，图中场景缓慢自然变成下一场景伴随着图中人物从当前场景缓慢变成下一个场景的人物，人物不要走动，人物只进行形态和样貌上的变化，场景变成下一场景，同时人物变换为下一场景的主要人物，动态转换流畅自然"
  return title.map((one, index) => {
    return {
      title: one,
      prompt: `在这张画布上生成图片，画面色彩以暖棕黄为主调，人物特写镜头，比例9:16，${originTitle[index]},${prompt[index]}，图片中不要显示任何文字，参考图片跟生成的人物图片, 人物正脸全身照，参考生图的人物比例要协调`,
      shot: customShot,
      lines‌: lines‌[index],
    };
  });
