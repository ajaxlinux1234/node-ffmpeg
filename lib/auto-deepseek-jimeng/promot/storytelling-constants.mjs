...
export const STORYTELLING_HISTORY = `${prompt}，{{name}}, 根据主题${name}现在{{timeNum}}个关键时间段, 时间点是从${name}开始到${name}结束, {{timeNum}}段视频生成提示词, 以及各个镜头画面之间的转换方式或运动方式, 视频镜头要是电影写实风格,比例9:16, 各段视频描述要与{{name}}的长相类似, 各段视频描述要写上人物年龄, 视频提示词不要显示国徽, 人民大会堂等政治信息, 严格生成{{timeNum}}段视频生成提示词，提示词为中文，每句话前面都加上"生成图片要符合实际生活场景", 所有片段场景不要包含人民大会堂`;
...
export const storytelling_deepseek_result_txt_fn = () => {
  ...
  const fullPrompt = takeRight(
    [...document.querySelectorAll("p.ds-markdown-paragraph")]
      .map((one) => one.innerText)
      .filter((one) => one.includes("画面提示") && one.includes("运镜方式")),
    historyNum
  );
