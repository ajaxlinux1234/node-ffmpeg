...
export async function runJimengVideoFlow(jimengVideo, processedData, name) {
  ...
  if (autoUpscale) {
    ...
    try {
      const upscaleResult = await upscaleImagesInDirectory(imagesDir, upscaleConfig);
  ...
  console.log(`  - allImageFiles: ${JSON.stringify(allImageFiles)} (类型: ${typeof allImageFiles})`);
  console.log(`  - imageFiles: ${JSON.stringify(imageFiles)} (类型: ${typeof imageFiles})`);
    ...
    try {
      ...
      if (firstFrameClicked) {
        ...
        if (fileInput) {
          ...
          console.log(`🔍 路径类型检查:`, fullPaths.map(p => typeof p));

          // 验证所有路径都是字符串
          const invalidPaths = fullPaths.filter(p => typeof p !== 'string');
          if (invalidPaths.length > 0) {
            console.error(`❌ 发现非字符串路径:`, invalidPaths);
            throw new Error(`路径必须是字符串，但发现: ${invalidPaths.map(p => typeof p).join(', ')}`);
          }
