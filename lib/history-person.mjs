...
async function generateThumbnail(videoPath, title, outputDir) {
  ...
  try {
    ...
    execCommand(`ffmpeg -y -ss 0.1 -i "${videoPath}" -vf "subtitles='${tempAssPath.replace(/\\/g, '/')}'" -vframes 1 "${thumbnailPath}"`);
...
async function addSectionTitles(videoPath, sectionTitles, outputDir, watermark) {
  ...
  try {
    ...
    // Outline=4, Shadow=4 适中可读性；颜色改为紫色，左对齐距离边缘20px；顶部边距60px
    // Alignment=7 表示左上对齐，MarginL=20 左边距20px，MarginR=20 右边距20px，WrapStyle=2 智能换行
    // 加粗字体Bold=1，字体大小适中确保完整显示
    const largerFontSize = Math.min(fontSize + 4, Math.floor(W / 25)); // 字体大小不超过屏幕宽度的1/25，确保能完整显示更多文本
    ass += `Style: Title, Arial Black, ${largerFontSize}, &H00FF00FF, &H0000FFFF, &H80000000, &H80000000, 1, 0, 0, 0, 100, 100, 0, 0, 1, 4, 4, 7, 20, 20, 60, 0\n\n`;
    ...
    const estCharWidthFactor = 1.2; // 每字符大约占用 fontSize * 1.2 宽度（更保守的估计，确保不超出屏幕）
    const safePadding = 100; // 左边距30px + 右边距预留70px，确保有足够安全区域
    const usableWidth = Math.max(200, W - 30 - safePadding); // 去除左边距30px和右边距，留出更大安全区
    ...
    for (let i = 0; i < sectionTitles.length; i++) {
      const start = i * 3;
      const end = start + 3.0; // 4 秒展示时间（提高可见性）
      ...
      const effect = `{\\fad(300,300)\\move(${startX},60,${endX},60,0,400)\\t(0,400,\\fscx110\\fscy110)\\t(400,800,\\fscx100\\fscy100)\\c&H00FF00FF&\\3c&H00000000&\\t(800,1500,\\c&HFF00AA&\\3c&H00000000&)\\t(1500,2500,\\c&H00FF00FF&\\3c&H00000000&)}`;
    ...
    // 使用最简单可靠的drawtext方案，一次只添加一个字幕
    console.log(`[3/5] 使用简化的drawtext方案逐个添加字幕...`);

    // 选择一个可用的中文字体，优先微软雅黑/黑体
    let cjkFontPath = '';
    const candidateFonts = [
      process.platform === 'win32' ? 'C:/Windows/Fonts/msyh.ttc' : '',      // 微软雅黑
      process.platform === 'win32' ? 'C:/Windows/Fonts/simhei.ttf' : '',    // 黑体
      process.platform === 'win32' ? 'C:/Windows/Fonts/msjh.ttc' : '',      // 微软正黑体
      process.platform === 'win32' ? 'C:/Windows/Fonts/simsun.ttc' : '',    // 宋体
      process.platform === 'win32' ? 'C:/Windows/Fonts/arial.ttf' : '',     // Arial兜底
    ].filter(Boolean);
    for (const fp of candidateFonts) {
      try {
        // 使用 pathExistsSync 不可用，则用 await 并在循环前声明函数环境为 async 已满足
        // 这里逐个检查存在性
        // 注意：在 Windows 上路径用正斜杠也可被 ffmpeg 接受
        // eslint-disable-next-line no-await-in-loop
        if (await fs.pathExists(fp)) { cjkFontPath = fp.replace(/\\/g, '/'); break; }
      } catch {}
    }
    if (!cjkFontPath) {
      // 兜底：仍然尝试 Arial，但可能无法显示中文
      cjkFontPath = process.platform === 'win32' ? 'C:/Windows/Fonts/arial.ttf' : '/System/Library/Fonts/Arial.ttf';
    }
    console.log(`[3/5] 使用字体: ${cjkFontPath}`);

    // 获取视频时长，合理分配字幕时间
    const durationProbe = execCommand(`ffprobe -v quiet -print_format json -show_format "${videoPath}"`);
    const formatInfo = JSON.parse(durationProbe.stdout);
    const videoDuration = parseFloat(formatInfo.format.duration) || 40;
    
    // 计算每个字幕的时间间隔，确保所有字幕都在视频时长内
    const subtitleDuration = 3.0; // 每个字幕显示4秒
    const totalSubtitles = sectionTitles.length;
    const availableTime = videoDuration - subtitleDuration; // 减去最后一个字幕的显示时间
    const interval = totalSubtitles > 1 ? availableTime / (totalSubtitles - 1) : 0;
    
    console.log(`[3/5] 视频时长: ${videoDuration}s, 字幕数量: ${totalSubtitles}, 时间间隔: ${interval.toFixed(2)}s`);
    ...
    for (let i = 0; i < sectionTitles.length; i++) {
      ...
      const end = Math.min(start + subtitleDuration, videoDuration); // 确保不超出视频时长
      const originalText = String(sectionTitles[i]).trim();
      
      // 改进换行：若文本中无空格（中文常见），按字符宽度硬换行；有空格则按词组换行
      let displayText = originalText;
      const hasSpace = /\s/.test(originalText);
      if (originalText.length > maxCharsPerLine) {
        const lines = [];
        if (hasSpace) {
          ...
          while (idx < originalText.length) {
            lines.push(originalText.slice(idx, idx + maxCharsPerLine));
            idx += maxCharsPerLine;
          }
        }
        // 不添加省略号，最多 4 行，超出的行继续换行但 drawtext 会整体显示
      ...
      const stepVideoPath = path.join(outputDir, `${videoName}_step${i+1}.mp4`);
      
      try {
        console.log(`[3/5] 添加第${i+1}个字幕: "${originalText}" (${start}s-${end}s)`);
        
        // 使用明确字体文件，并启用文本盒子背景，避免各种播放器下渲染差异
        // 注意：Windows路径中的冒号需要转义，使用双反斜杠转义冒号
        const escapedFontPath = cjkFontPath.replace(/:/g, '\\:');
        // 创建美观的复合字体效果：渐变色彩 + 阴影 + 描边
        const drawTextCmd = `ffmpeg -y -i "${currentVideoPath}" -vf "drawtext=text='${safeText}':fontcolor=0xFFD700:fontsize=${largerFontSize}:fontfile='${escapedFontPath}':x=40:y=80:shadowcolor=0x000000:shadowx=3:shadowy=3:bordercolor=0x8B0000:borderw=2:enable='between(t,${start},${end})'" -c:v libx264 -pix_fmt yuv420p -c:a copy "${stepVideoPath}"`;
        
        console.log(`[3/5] 执行命令: ${drawTextCmd.substring(0, 100)}...`);
        execCommand(drawTextCmd);
        
        // 验证输出文件
        const stepExists = await fs.pathExists(stepVideoPath);
        if (stepExists) {
          console.log(`[3/5] 第${i+1}个字幕添加成功`);
          // 清理上一步的临时文件（除了原始视频）
          if (currentVideoPath !== videoPath) {
            await fs.remove(currentVideoPath).catch(() => {});
          }
          currentVideoPath = stepVideoPath;
          subtitleCount++;
        } else {
          console.warn(`[3/5] 第${i+1}个字幕添加失败，文件不存在，跳过继续处理下一个`);
          // 不要break，继续处理下一个字幕，但保持当前视频路径不变
        }
        
      } catch (error) {
        console.warn(`[3/5] 第${i+1}个字幕添加失败: ${error.message}，跳过继续处理下一个`);
        // 不要break，继续处理下一个字幕
      }
    }
    
    // 将最终结果复制到目标路径
    if (subtitleCount > 0 && currentVideoPath !== videoPath) {
      try {
        console.log(`[3/5] 成功添加了${subtitleCount}个字幕，复制到最终路径...`);
        execCommand(`ffmpeg -y -i "${currentVideoPath}" -c copy "${tempVideoPath}"`);
        
        // 清理最后一个步骤文件
        await fs.remove(currentVideoPath).catch(() => {});
        
        console.log(`[3/5] 字幕视频生成完成: ${tempVideoPath}`);
      } catch (copyError) {
        console.warn(`[3/5] 复制最终视频失败: ${copyError.message}`);
        // 如果复制失败，直接使用当前视频路径
        if (currentVideoPath !== videoPath) {
          try {
            await fs.move(currentVideoPath, tempVideoPath);
          } catch (moveError) {
            console.warn(`[3/5] 移动视频文件失败: ${moveError.message}`);
            return videoPath;
          }
        }
      }
    } else {
      console.warn(`[3/5] 没有成功添加任何字幕，使用原视频`);
      execCommand(`ffmpeg -y -i "${videoPath}" -c copy "${tempVideoPath}"`);
    }
