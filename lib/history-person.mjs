...
/**
 * 生成优化的FFmpeg编码参数
 * @param {string} qualityMode - 质量模式: 'high', 'balanced', 'fast'
 * @returns {string} - FFmpeg编码参数字符串
 */
function getOptimizedEncodingParams(qualityMode = 'high') {
  const baseParams = `-c:v ${CONFIG_VIDEO.VIDEO_CODEC} -pix_fmt ${CONFIG_VIDEO.PIXEL_FORMAT}`;
  
  switch (qualityMode) {
    case 'high':
      // 高质量模式：接近无损，文件稍大但质量最佳
      return `${baseParams} -crf ${CONFIG_VIDEO.CRF_VALUE} -preset ${CONFIG_VIDEO.PRESET_SLOW} -profile:v ${CONFIG_VIDEO.VIDEO_CODEC_PROFILE} -level ${CONFIG_VIDEO.VIDEO_CODEC_LEVEL} -tune ${CONFIG_VIDEO.TUNE_OPTION} -me_method ${CONFIG_VIDEO.MOTION_ESTIMATION} -subq ${CONFIG_VIDEO.SUBPIXEL_REFINEMENT} -refs ${CONFIG_VIDEO.REFERENCE_FRAMES} -maxrate ${CONFIG_VIDEO.MAX_BITRATE} -bufsize ${CONFIG_VIDEO.BUFFER_SIZE}`;
    
    case 'balanced':
      // 平衡模式：质量与文件大小平衡
      return `${baseParams} -crf ${CONFIG_VIDEO.CRF_VALUE_BALANCED} -preset ${CONFIG_VIDEO.PRESET_MEDIUM} -profile:v ${CONFIG_VIDEO.VIDEO_CODEC_PROFILE} -tune ${CONFIG_VIDEO.TUNE_OPTION}`;
    
    case 'fast':
      // 快速模式：处理速度优先
      return `${baseParams} -crf ${CONFIG_VIDEO.CRF_VALUE_BALANCED} -preset ${CONFIG_VIDEO.PRESET_FAST}`;
    
    default:
      return `${baseParams} -crf ${CONFIG_VIDEO.CRF_VALUE} -preset ${CONFIG_VIDEO.PRESET_SLOW}`;
  }
}
...
async function applyAIEnhance(videoPath, outputDir, options) {
  ...
  if (hasRealesrgan) {
    ...
    if (await fs.pathExists(tmpEnhanced)) {
      const encodingParams = getOptimizedEncodingParams(options?.qualityMode || 'high');
      execCommand(
...
async function addBlurMask(videoPath, outputDir, opt) {
  ...
  const encodingParams = getOptimizedEncodingParams(opt?.qualityMode || 'high');
  execCommand(
...
async function addSectionTitles(
  ...
  try {
    ...
    for (let i = 0; i < sectionTitles.length; i++) {
      ...
      const end = Math.min(start + CONFIG_SUBTITLE.SECTION_TITLE_DURATION - 1, duration); // 确保不超出视频时长
    ...
    const assPathEscaped = path
      ...
      .replace(/\\/g, "/");
    const encodingParams = getOptimizedEncodingParams(qualityMode || 'high');
 ...
 */
async function compositeWithMusic(videoPath, bgMusicPath, outputDir, qualityMode) {
  await fs.mkdir(outputDir, { recursive: true });
  ...
  if (useBgMusic) {
    ...
    const encodingParams = getOptimizedEncodingParams(qualityMode || 'high');
    execCommand(
      `ffmpeg -y -i "${videoPath}" -i "${resolvedBgMusicPath}" -map 0:v -map 1:a:0 ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} -shortest "${finalVideoPath}"`
    );
  } else {
...
function extractSectionInfo(prompt, name, index) {
  ...
  if (sceneMatch) {
    ...
    if (rawScene.length > 0) {
      // 智能截取：优先在词汇边界截取
      const keyPlaces = ['学堂', '私塾', '学校', '大学', '实验室', '研究所', '会场', '田间', '稻田', '基地', '农场', '家中', '书房', '卧房'];
      let foundPlace = keyPlaces.find(place => rawScene.includes(place));
      
      if (foundPlace) {
        const placeIndex = rawScene.indexOf(foundPlace);
        const beforePlace = rawScene.substring(0, placeIndex + foundPlace.length);
        scene = beforePlace.length <= 12 ? beforePlace : rawScene.substring(0, 10);
      } else {
  ...
  if (index === 0) {
    ...
    // 高级事件提取算法
    const cleanPrompt = prompt.replace(/[，,。！？；：]/g, ' ').replace(/\s+/g, ' ');
    
    // 动作关键词匹配（按优先级排序）
    const actionPatterns = [
      // 学习相关
      { pattern: /(临摹|抄写|练习|学习|研读|阅读)[^，,。]*?([《》\w\u4e00-\u9fff]+)/g, priority: 1 },
      { pattern: /(求学|上学|入学|就读|学习)/g, priority: 2 },
      
      // 工作研究相关
      { pattern: /(发明|创造|研制|开发|设计)[^，,。]*?([核弹|原子弹|氢弹|武器|装置|\w\u4e00-\u9fff]+)/g, priority: 1 },
      { pattern: /(研究|实验|观察|分析|测试|检验)[^，,。]*?([稻穗|水稻|植物|数据|样本|\w\u4e00-\u9fff]*)/g, priority: 1 },
      { pattern: /(指导|教学|演讲|报告|讲解|传授)/g, priority: 2 },
      
      // 生活事件
      { pattern: /(获得|荣获|赢得|获奖)[^，,。]*?([奖项|荣誉|称号|\w\u4e00-\u9fff]+)/g, priority: 1 },
      { pattern: /(参加|出席|参与)[^，,。]*?([会议|活动|典礼|\w\u4e00-\u9fff]+)/g, priority: 1 },
      { pattern: /(庆祝|分享|欢度)[^，,。]*?([生日|节日|\w\u4e00-\u9fff]*)/g, priority: 1 },
    ];
    ...
    if (bestMatch) {
      ...
      // 降级处理：从场景推断事件
      if (scene.includes('私塾') || scene.includes('学堂') || scene.includes('学校')) {
        event = '求学';
        ...
        // 最后尝试从原文提取关键动词
        const verbMatch = cleanPrompt.match(/(做|进行|从事|完成|实现|达成)([^，,。\s]{2,8})/);
        if (verbMatch) {
          event = verbMatch[2];
        }
      }
...
async function updateConfigFile(sectionTitle, name) {
  try {
    ...
    const sectionTitleStr = sectionTitle.map(title => `      "${title.replace(/\n/g, '\\n')}"`).join(',\n');
    ...
    if (sectionTitleRegex.test(configContent)) {
      configContent = configContent.replace(sectionTitleRegex, newSectionTitleBlock);
    ...
    console.log(`✅ 已成功将${name}的${sectionTitle.length}个分镜标题写入配置文件`);
...
export default async function runHistoryPerson(config) {
  ...
  let { url, title, sectionTitle, "bg-music": bgMusic, watermark, titleAnimation, sectionTitleAnimation, name, qualityMode } = config;
  ...
  console.log(`视频质量模式: ${qualityMode || "high"} (high=高质量接近无损, balanced=平衡, fast=快速)`);
  console.log(`当前平台: ${process.platform}, 使用字体: ${getPlatformFont()}`);

  // 检查配置文件中是否已有手动优化的sectionTitle
  const hasManualSectionTitle = sectionTitle && sectionTitle.length > 0 && 
    sectionTitle.some(title => !title.includes('重要时刻') && !title.includes('实验室'));
  ...
  if (hasManualSectionTitle) {
    ...
    try {
      const processedDataPath = path.join(process.cwd(), 'output', name, 'processed_data.json');
      if (await fs.pathExists(processedDataPath)) {
        ...
        if (processedData.segments && processedData.segments.length > 0) {
          ...
          for (let i = 0; i < processedData.segments.length; i++) {
            ...
            const { year, age, scene, event } = extractSectionInfo(prompt, name, i);
            
            const sectionText = `${year}年/${age}岁\n${scene}\n${event}`;
            sectionTitle.push(sectionText);
            
            console.log(`📝 生成第${i+1}个分镜: ${sectionText.replace(/\n/g, ' | ')}`);
          }
          
          console.log(`✅ 成功从processed_data.json生成了${sectionTitle.length}个分镜标题\n`);
          
          // 将生成的sectionTitle写入配置文件
          await updateConfigFile(sectionTitle, name);
        }
      } else {
        console.log(`⚠️ 未找到 ${name} 的processed_data.json文件，将使用配置中的sectionTitle\n`);
      }
    } catch (error) {
      console.warn(`[警告] 从processed_data.json生成sectionTitle失败: ${error.message}\n`);
    }
  }
