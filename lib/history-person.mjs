...
async function applyAIEnhance(videoPath, outputDir, options) {
  ...
  try {
    await $`which realesrgan-ncnn-vulkan`;
    hasRealesrgan = true;
  } catch {}
  ...
  if (hasRealesrgan) {
    ...
    try {
      await $`realesrgan-ncnn-vulkan -i ${videoPath} -o ${tmpEnhanced} -n ${model} -s ${scale}`;
    } catch (e) {
    ...
    if (await fs.pathExists(tmpEnhanced)) {
      await $`ffmpeg -y -i ${tmpEnhanced} -i ${videoPath} -map 0:v -map 1:a? -c:v libx264 -pix_fmt yuv420p -crf 18 -preset medium -c:a copy ${outPath}`;
      // 清理临时
  ...
  await $`ffmpeg -y -i ${videoPath} -vf ${vf} -c:v libx264 -pix_fmt yuv420p -crf 18 -preset medium -c:a copy ${outPath}`;
...
async function applyBlurMask(videoPath, outputDir, opt) {
  ...
  // 获取帧尺寸用于生成 mask 尺寸
  const probe = await $`ffprobe -v quiet -print_format json -show_streams ${videoPath}`;
...
`;
  ...
  await $`ffmpeg -y -i ${videoPath} -filter_complex ${filter} -map "[out]" -map 0:a? -c:v libx264 -pix_fmt yuv420p -crf 18 -preset medium -c:a copy ${outPath}`;
  return outPath;
}
...
async function standardizeAspect(videoPath, outputDir, options) {
  ...
  await $`ffmpeg -y -i ${videoPath} -vf ${filter} -c:v libx264 -pix_fmt yuv420p -crf 18 -preset medium -c:a copy ${outPath}`;
...
async function addSectionTitles(videoPath, sectionTitles, outputDir, watermark) {
  ...
  try {
    ...
    // 提高左安全边距以适配平台可能的自动裁剪/遮挡；顶部边距 95px
    ass += `Style: Title, PingFang SC, ${fontSize}, &H00FFFF00, &H0000FFFF, &H80000000, &H80000000, 0, 0, 0, 0, 100, 100, 0, 0, 1, 3, 4, 7, 60, 20, 80, 0\n\n`;
...
export default async function runHistoryPerson(config) {
  ...
  try {
    ...
    const temps = [videoWithTitles];
    if (finalVideoPath !== composedPath) temps.push(composedPath);
    if (finalVideoPath !== maskedPath && maskedPath !== composedPath) temps.push(maskedPath);
    if (finalVideoPath !== enhancedPath && enhancedPath !== maskedPath) temps.push(enhancedPath);
