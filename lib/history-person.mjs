...
`;
  ...
  execCommand(`ffmpeg -y -i "${videoPath}" -filter_complex "${filter}" -map "[out]" -map 0:a? -c:v ${CONFIG_VIDEO.VIDEO_CODEC} -pix_fmt ${CONFIG_VIDEO.PIXEL_FORMAT} -crf ${CONFIG_VIDEO.CRF_VALUE} -preset ${CONFIG_VIDEO.PRESET_MEDIUM} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_COPY} "${outPath}"`);
  return outPath;
}
...
async function standardizeAspect(videoPath, outputDir, options) {
  const { targetAspect = CONFIG_VIDEO.DEFAULT_ASPECT_RATIO, targetResolution = CONFIG_VIDEO.DEFAULT_RESOLUTION, fitMode = CONFIG_VIDEO.DEFAULT_FIT_MODE } = options || {};
  if (fitMode !== 'crop') {
  ...
  execCommand(`ffmpeg -y -i "${videoPath}" -vf "${filter}" -c:v ${CONFIG_VIDEO.VIDEO_CODEC} -pix_fmt ${CONFIG_VIDEO.PIXEL_FORMAT} -crf ${CONFIG_VIDEO.CRF_VALUE} -preset ${CONFIG_VIDEO.PRESET_MEDIUM} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_COPY} "${outPath}"`);
...
async function generateThumbnail(videoPath, title, outputDir) {
  ...
  try {
    ...
    // 创建临时 ASS 字幕文件用于封面标题
    const tempAssPath = path.join(outputDir, `${videoName}_cover_title${CONFIG_PATHS.ASS_SUFFIX}`);
 ...
 */
async function addSectionTitles(videoPath, sectionTitles, outputDir, watermark, topTitle) {
  ...
  try {
    ...
    const assPath = path.join(outputDir, `${videoName}${CONFIG_PATHS.ASS_SUFFIX}`);
    let ass = '';
    ass += `[Script Info]\n`;
    ass += `ScriptType: v4.00+\n`;
    ass += `PlayResX: ${W}\n`;
    ...
    ass += `Style: TopTitleKeyword, KaiTi, 60, &H80FF0000, &H80FF0000, &H00000000, &H80000000, 1, 0, 0, 0, 100, 100, 0, 0, 1, 4, 0, 2, 80, 80, 0, 0\n\n`;
    ...
    const usableWidth = Math.max(CONFIG_SUBTITLE.MIN_USABLE_WIDTH, W - safePadding); // 去除左右边距，确保100px边距
    const maxCharsPerLine = Math.max(CONFIG_SUBTITLE.MIN_CHARS_PER_LINE, Math.floor(usableWidth / (largerFontSize * estCharWidthFactor))); // 最小3字符，确保能显示
    ...
    for (let i = 0; i < sectionTitles.length; i++) {
      ...
      for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        ...
        for (let j = 0; j < chars.length; j++) {
          ...
          const baseY = H - Math.floor(H * CONFIG_SUBTITLE.SUBTITLE_POSITION_Y_PERCENT) + lineIndex * (largerFontSize + 10); // 从下往上40%位置，每行间隔
          ...
          for (let k = 0; k < j; k++) {
            ...
            const isNextCharDigit = k < chars.length - 1 ? /\d/.test(chars[k + 1]) : false;
          ...
          const effect = `{\\pos(${charX},${baseY})\\fad(${CONFIG_SUBTITLE.FADE_IN_OUT_MS},${CONFIG_SUBTITLE.FADE_IN_OUT_MS * 6})\\t(0,${CONFIG_SUBTITLE.SCALE_DURATION_MS},\\fscx${CONFIG_SUBTITLE.SCALE_X_START}\\fscy${CONFIG_SUBTITLE.SCALE_Y_START})\\t(${CONFIG_SUBTITLE.SCALE_DURATION_MS},${CONFIG_SUBTITLE.SCALE_END_MS},\\fscx${CONFIG_SUBTITLE.SCALE_X_END}\\fscy${CONFIG_SUBTITLE.SCALE_Y_END})}`;
          
          ass += `Dialogue: 0,${toAssTime(charStart)},${toAssTime(charEnd)},${styleName},,0,0,0,,${effect}${escapedChar}\n`;
          
          totalCharIndex++;
        }
      }
      
      console.log(`[3/5] 添加打字机效果字幕 ${i+1}/${sectionTitles.length}: "${sectionTitles[i]}" -> ${totalCharIndex}个字符 (${start}s-${end}s)`);
    }
    ...
    if (watermark && duration > 0) {
      ...
      const tEnd = Math.max(0, Math.floor(duration * 1000) - CONFIG_WATERMARK.STAY_END_MS); // 倒数1s 开始停留
      const travelMs = Math.max(0, tEnd - t1);
      const halfMs = Math.floor(travelMs / CONFIG_WATERMARK.HALF_WAY_FACTOR);
      // 0-1s：左下角停留（精确 100px）
      ass += `Dialogue: 0,${toAssTime(0)},${toAssTime(1)},WM_BL,,0,0,0,,{\\pos(${CONFIG_SUBTITLE.LEFT_MARGIN},${H - CONFIG_SUBTITLE.LEFT_MARGIN})}${wmEsc}\n`;
      // 1s-中点：BL -> Mid
      ass += `Dialogue: 0,${toAssTime(1)},${toAssTime(1 + travelMs/CONFIG_WATERMARK.TRAVEL_DURATION_FACTOR)},WM_M,,0,0,0,,{\\move(${cx1},${cy1},${mx},${my},0,${halfMs})}${wmEsc}\n`;
      // 中点-最后1s：Mid -> TR
      ass += `Dialogue: 0,${toAssTime(1 + travelMs/CONFIG_WATERMARK.TRAVEL_DURATION_FACTOR)},${toAssTime(Math.max(1, duration - 1))},WM_M,,0,0,0,,{\\move(${mx},${my},${cx2},${cy2},0,${Math.max(0, travelMs - halfMs)})}${wmEsc}\n`;
      // 最后1s：右上角停留（精确 100px）
      const endStart = Math.max(0, duration - 1.0);
      ass += `Dialogue: 0,${toAssTime(endStart)},${toAssTime(duration)},WM_TR,,0,0,0,,{\\pos(${W - CONFIG_SUBTITLE.RIGHT_MARGIN},${CONFIG_SUBTITLE.RIGHT_MARGIN})}${wmEsc}\n`;
    }
    ...
    if (topTitle && duration > 0) {
      ...
      const topTitleY = Math.floor(H * 0.2);
      ...
      for (let lineIndex = 0; lineIndex < topTitleLines.length; lineIndex++) {
        ...
        for (let charIndex = 0; charIndex < lineChars.length; charIndex++) {
          ...
          // 确定字符样式：根据字符在原文中的位置决定是否为关键词
          const globalCharIndex = topTitleCleanCharIndex;
          ...
          // 贯穿全视频的标题
          ass += `Dialogue: 0,${toAssTime(0)},${toAssTime(duration)},${styleName},,0,0,0,,${finalFlashEffect}${escapedChar}\n`;
      ...
      console.log(`[3/5] 顶部标题已添加: "${topTitle}" (贯穿全视频，${topTitleLines.length}行，${topTitleCleanCharIndex}个字符)`);
    }
...
async function compositeWithMusic(videoPath, bgMusicPath, outputDir) {
  ...
  const finalVideoPath = path.join(outputDir, `${cleanVideoName}${CONFIG_PATHS.PROCESSED_SUFFIX}.mp4`);
  ...
  if (useBgMusic) {
    ...
    // 保持原始比例，不进行缩放
    execCommand(`ffmpeg -y -i "${videoPath}" -i "${resolvedBgMusicPath}" -map 0:v -map 1:a:0 -c:v ${CONFIG_VIDEO.VIDEO_CODEC} -pix_fmt ${CONFIG_VIDEO.PIXEL_FORMAT} -crf ${CONFIG_VIDEO.CRF_VALUE} -preset ${CONFIG_VIDEO.PRESET_MEDIUM} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} -shortest "${finalVideoPath}"`);
  } else {
    console.log(`[4/5] 不使用背景音乐，仅处理视频`);
    execCommand(`ffmpeg -y -i "${videoPath}" -map 0:v -map 0:a? -c:v ${CONFIG_VIDEO.VIDEO_CODEC} -pix_fmt ${CONFIG_VIDEO.PIXEL_FORMAT} -crf ${CONFIG_VIDEO.CRF_VALUE} -preset ${CONFIG_VIDEO.PRESET_MEDIUM} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_COPY} "${finalVideoPath}"`);
  }
...
async function cleanupOutputDir(outputDir, keepPattern) {
  try {
    ...
    for (const file of files) {
      ...
      // 删除旧的处理文件、验证截图和临时文件
      if (file.includes(CONFIG_CLEANUP.OLD_FINAL_PATTERN) || 
          ...
          (file.includes(CONFIG_PATHS.PROCESSED_SUFFIX) && !file.includes(keepPattern))) {
...
export default async function runHistoryPerson(config) {
  ...
  // 可选：强制输出为指定比例与分辨率
  const targetAspect = config.target_aspect || CONFIG_VIDEO.DEFAULT_ASPECT_RATIO;
  const targetResolution = config.target_resolution || CONFIG_VIDEO.DEFAULT_RESOLUTION;
  ...
  try {
    ...
    // 3. 添加镜头标题
    const videoWithTitles = await addSectionTitles(videoPath, sectionTitle, outputDir, watermark, title);
