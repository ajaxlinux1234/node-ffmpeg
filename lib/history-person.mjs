...
async function addSectionTitles(videoPath, sectionTitles, outputDir, watermark) {
  ...
  try {
    ...
    ass += `Style: Title, Arial Black, ${largerFontSize}, &H0000FFFF, &H0000FFFF, &H80000000, &H80000000, 1, 0, 0, 0, 100, 100, 0, 0, 1, 3, 2, 1, 0, 0, 0, 0\n\n`;
    // 水印样式（普通字体白色，极低不透明度≈0.02 -> AA≈FA），起点/中段/终点；去除描边和阴影
    // 注意：ASS 颜色为 &HAABBGGRR，AA 越接近 FF 越透明
    ass += `Style: WM_BL, Arial, 20, &HD0FFFFFF, &HD0FFFFFF, &H00000000, &H00000000, 0, 0, 0, 0, 100, 100, 0, 0, 1, 0, 0, 1, 20, 20, 20, 0\n`;
    ass += `Style: WM_M,  Arial, 20, &HD0FFFFFF, &HD0FFFFFF, &H00000000, &H00000000, 0, 0, 0, 0, 100, 100, 0, 0, 1, 0, 0, 5, 20, 20, 20, 0\n`;
    ass += `Style: WM_TR, Arial, 20, &HD0FFFFFF, &HD0FFFFFF, &H00000000, &H00000000, 0, 0, 0, 0, 100, 100, 0, 0, 1, 0, 0, 9, 20, 20, 20, 0\n`;
    ...
    const safePadding = 120; // 左边距60px + 右边距60px
    const usableWidth = Math.max(200, W - safePadding); // 去除左右边距，确保60px边距
    ...
    for (let i = 0; i < sectionTitles.length; i++) {
      ...
      // 打字机效果：逐个字符从左到右显示，使用绝对定位
      const text = wrapped.replace(/\\N/g, '\n'); // 保留换行符以便处理多行
      const lines = text.split('\n');
      const typewriterSpeed = 0.08; // 每个字符显示间隔（秒）
      ...
      for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        ...
        for (let j = 0; j < chars.length; j++) {
          ...
          // 计算字符位置：60px左边距 + 字符偏移
          const baseX = 60; // 严格60px左边距
          const baseY = H - Math.floor(H * 0.4) + lineIndex * (largerFontSize + 10); // 从下往上40%位置，每行间隔
          ...
          // 检查是否超出右边距60px
          if (charX + largerFontSize * 0.8 > W - 60) {
          ...
          const effect = `{\\pos(${charX},${baseY})\\fad(50,300)\\t(0,200,\\fscx120\\fscy120)\\t(200,400,\\fscx100\\fscy100)}`;
          
          ass += `Dialogue: 0,${toAssTime(charStart)},${toAssTime(charEnd)},Title,,0,0,0,,${effect}${escapedChar}\n`;
