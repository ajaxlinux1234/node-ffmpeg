...
async function generateCover(projectName, title, outputDir) {
  ...
  // 1. 构建青年图片路径
  const youthImagePath = path.join(process.cwd(), "output", projectName, "青年.jpeg");
  ...
  try {
    ...
    console.log(`[封面生成] 图片尺寸: ${imageWidth}x${imageHeight}, 文字位置Y: ${textY}`);
    ...
    try {
      // 首先尝试中文字符
      let magickCommand = `magick "${youthImagePath}" -pointsize 80 -fill white -stroke black -strokewidth 2 -gravity center -annotate +0+0 "${personName}" "${coverPath}"`;
      ...
      if (Math.abs(newSize - originalSize) < 100) {
        // 文件大小几乎没变，可能中文字符没有正确渲染，尝试英文
        console.warn(`⚠️ 中文字符可能未正确渲染，尝试使用英文名称`);
        const englishName = personName === "李政道" ? "Li Zhengdao" : 
                           personName === "邓稼先" ? "Deng Jiaxian" :
                           personName === "袁隆平" ? "Yuan Longping" :
                           personName === "钱学森" ? "Qian Xuesen" :
        ...
        console.log(`✅ 使用中文名称 "${personName}" 生成封面成功`);
      }
