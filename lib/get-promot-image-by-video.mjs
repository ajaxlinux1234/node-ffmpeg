...
export async function runGetPromotImageByVideo(config, options = {}) {
  ...
  try {
    ...
    if (config.directDownload) {
      ...
      // 直接打开页面并执行 get_title_selector_fn
      console.log(`\n📋 直接从页面获取数据并生成 processed_data.json`);
      ...
      try {
        ...
        if (Array.isArray(results) && results.length > 0) {
          console.log(`✅ 获取到 ${results.length} 个标题`);
          results.forEach((result, index) => {
            console.log(`   第 ${index + 1} 个: "${result?.substring(0, 50)}${result?.length > 50 ? "..." : ""}"`);
          });

          // 创建 processed_data.json
          ...
          console.log(`⚠️ 未获取到有效数据，结果: ${typeof results}`);
        }
      } catch (error) {
        console.error(`❌ 执行选择器函数失败:`, error.message);
      }
