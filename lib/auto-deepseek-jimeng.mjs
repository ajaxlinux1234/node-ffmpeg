...
async function updateConfigSectionTitle(extractedData, name) {
  try {
    ...
    if (needsUpdate) {
      ...
      const sectionTitleArray = newTitles.map((title) => {
...
async function runJimengVideoFlow(jimengVideo, processedData, name) {
  ...
  // 根据 generate_section 参数选择要上传的图片和运镜描述
  let imageFiles = [];
  let shots = [];
  
  if (generate_section === 1) {
    // 第1批：倒序的前10张图片 (image_16 到 image_7)
    imageFiles = allImageFiles.slice(0, 10);
    // 对应的运镜描述：索引 0-9
    shots = processedData.segments.slice(0, 10).map((seg) => seg.shot).filter(Boolean);
    console.log(`📸 第1批：上传图片 ${imageFiles[imageFiles.length-1]} 到 ${imageFiles[0]}`);
    console.log(`📝 第1批：运镜描述索引 0-9 (共 ${shots.length} 个)`);
  } else if (generate_section === 2) {
    // 第2批：从第1批的最后一张开始，继续倒序的剩余图片
    // 由于要保持连贯性，从 image_7 开始到 image_1
    const remainingImages = allImageFiles.slice(9); // 从第10张开始(image_7)
    imageFiles = remainingImages.slice(0, 10); // 最多取10张
    // 对应的运镜描述：索引 10-15
    shots = processedData.segments.slice(10).map((seg) => seg.shot).filter(Boolean);
    console.log(`📸 第2批：上传图片 ${imageFiles[imageFiles.length-1]} 到 ${imageFiles[0]}`);
    console.log(`📝 第2批：运镜描述索引 10-${processedData.segments.length-1} (共 ${shots.length} 个)`);
  } else {
    throw new Error(`不支持的 generate_section 值: ${generate_section}，只支持 1 或 2`);
  }
