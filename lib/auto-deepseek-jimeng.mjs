...
async function runJimengFlow(jimeng, processedData) {
  ...
  if (persistLogin) {
    ...
    launchConfig.userDataDir = jimengUserDataDir;
    console.log(`🔐 启用登录状态持久化，账号 ${accountId} 数据保存在:`, jimengUserDataDir);
  } else {
    console.log(`🔓 未启用登录状态持久化，账号 ${accountId} 每次都需要重新登录`);
  }
  ...
  try {
    ...
    if (loginButton) {
      ...
      if (generateButton) {
        console.log("   - 确认：找到生成按钮");
      }
      if (userAvatar) {
        console.log("   - 确认：找到用户头像");
      }
    }
