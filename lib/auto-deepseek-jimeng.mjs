...
export async function clearBrowserData() {
  const deepseekUserDataDir = path.join(process.cwd(), "browser-data", "deepseek-profile");
  const jimengUserDataDir = path.join(process.cwd(), "browser-data", "jimeng-profile");
...
export default async function runAutoDeepseekJimeng(config) {
  ...
  // 检查本地是否已有数据
  const localDataExists = await checkLocalData(jimeng?.name || deepseek.send_msg_template_data?.name);
  
  let processedData;
  if (localDataExists) {
    console.log("✅ 发现本地数据，跳过 DeepSeek 步骤");
    processedData = await loadLocalData(jimeng?.name || deepseek.send_msg_template_data?.name);
  } else {
...
function mergeRelatedSegments(rawResults) {
  ...
  for (let i = 0; i < rawResults.length; i++) {
    ...
    const segmentMatch = text.match(/第(\d+)段[:：]\s*(\d{4})年[，,]?\s*(.+?)\s*\(年龄[:：]\s*约?(\d+)岁\)/);
    ...
    } else if (text.startsWith("视频生成提示词：") || text.includes("视频生成提示词:")) {
      // 这是视频生成提示词内容
      if (currentSegment) {
    ...
    } else if (text.startsWith("镜头转换/运动方式：") || text.includes("镜头转换/运动方式:")) {
      // 这是镜头转换/运动方式
      if (currentSegment) {
...
async function runJimengFlow(jimeng, processedData) {
  ...
  try {
    ...
    // 检查是否有登录按钮（如果有说明未登录）
    const loginButton = await page.$(login_selector?.login_button || '#SiderMenuLogin');
    if (loginButton) {
      ...
      // 检查是否有同意政策按钮
      const agreeButton = await page.$(login_selector?.agree_policy || 'div.zoomModal-enter-done .lv-btn-primary');
      if (agreeButton) {
    ...
    // 获取提示词列表
    const prompts = processedData.segments.map(segment => segment.prompt || segment.originalText);
    ...
    for (let i = 0; i < prompts.length; i++) {
      try {
        ...
        console.log(`📤 发送第 ${i + 1} 个提示词: ${prompt.substring(0, 50)}...`);
        ...
        try {
          ...
          // 如果回车键失败，尝试点击发送按钮
          try {
            await page.waitForSelector(img_generate_input_send_selector, { timeout: 3000 });
        ...
        // 如果是页面关闭错误，尝试重新打开
        if (error.message.includes('Target closed') || error.message.includes('页面已关闭')) {
    ...
    await page.waitForTimeout(10000); // 等待图片生成

    // 下载生成的图片
    console.log("📥 开始下载生成的图片...");
    await downloadGeneratedImages(page, gernerate_img_result_selector, prompts.length, jimeng.name);
 ...
 * 下载生成的图片（支持虚拟列表滚动）
 */
async function downloadGeneratedImages(page, resultSelector, count, name) {
  // 创建下载目录
  ...
  while (downloadedCount < count && scrollAttempts < maxScrollAttempts) {
    ...
    for (let i = 0; i < 4; i++) {
      try {
        ...
        // 检查URL是否有效且未下载过
        if (imgSrc && imgSrc.startsWith('http') && !downloadedUrls.has(imgSrc)) {
          downloadedUrls.add(imgSrc);
          downloadedCount++;
          
          const filename = `image_${downloadedCount}.jpg`;
          await downloadImageFromUrl(page, imgSrc, downloadDir, filename);
          console.log(`✅ 第 ${downloadedCount} 张图片下载完成 (${filename})`);
    ...
    if (downloadedCount < count) {
      console.log(`📤 尝试滚动加载更多图片... (已下载: ${downloadedCount}/${count})`);
 ...
 */
async function downloadImageFromUrl(page, imageUrl, downloadDir, filename) {
  try {
    ...
    for (let i = 0; i < images.length; i++) {
      try {
        ...
        if (width > 200 && height > 200 && src && src.startsWith('http')) {
          ...
          await downloadImageFromUrl(page, src, downloadDir, `generated_image_${downloadCount + 1}.jpg`);
          downloadCount++;
