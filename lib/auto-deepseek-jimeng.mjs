...
async function runJimengFlow(jimeng, processedData) {
  ...
  try {
    ...
    // 检查是否直接下载图片
    if (downloadImg) {
      console.log(`📥 直接下载模式：跳过生成，直接下载 ${prompts.length} 张图片`);
      ...
      while (downloadedCount < prompts.length && retryCount < maxRetries) {
        ...
        if (retryCount < maxRetries) {
          console.log(`⏳ 继续向上滚动寻找更多图片，继续重试... (${retryCount}/${maxRetries})`);
          ...
          // 极度温和的向上滚动，进一步减少次数和幅度
          for (let i = 0; i < 2; i++) {
            await page.mouse.wheel({ deltaY: -100 }); // 进一步减小向上滚动幅度
            console.log(`   📜 极度温和向上滚动 ${i + 1}/2`);
            await page.waitForTimeout(2000); // 进一步增加等待时间
          }
      ...
      if (downloadedCount < prompts.length) {
        console.warn(`⚠️ 预期下载 ${prompts.length} 张，实际下载 ${downloadedCount} 张`);
...
async function downloadGeneratedImages(
  ...
  while (downloadedCount < count && scrollAttempts < maxScrollAttempts) {
    ...
    if (downloadedCount < count) {
      ...
      // 极度温和的单次向上滚动
      await page.mouse.wheel({ deltaY: -80 }); // 进一步减小向上滚动幅度
      console.log(`   📜 极度温和向上滚动`);
...
async function removeExistingReferenceImage(
  ...
  try {
    console.log("🔍 检查是否存在已有的参考图片...");

    // 查找参考图片容器
    const imgContainer = await page.$(imgContainerSelector);
    if (!imgContainer) {
    ...
    // 检查容器中是否有图片
    const existingImg = await imgContainer.$("img");
    if (!existingImg) {
      console.log("✅ 参考图片容器中无图片，无需删除");
      return;
    }

    console.log("🗑️ 发现已有参考图片，开始删除...");
    ...
    // 在图片容器内查找删除按钮（更精确的查找）
    let closeButton = await imgContainer.$(
      imgCloseSelector.replace(imgContainerSelector + " ", "")
    );
    ...
    if (closeButton) {
      ...
      // 验证图片是否已删除
      const remainingImg = await page.$(imgContainerSelector + " img");
      if (!remainingImg) {
        console.log("✅ 参考图片已成功删除");
      } else {
        console.log("⚠️ 参考图片可能未完全删除，尝试再次删除...");

        // 尝试再次删除
        const retryCloseButton = await page.$(imgCloseSelector);
        if (retryCloseButton) {
          await retryCloseButton.click();
          await page.waitForTimeout(1000);
          console.log("🔄 执行了二次删除尝试");
        }
      }
    } else {
      console.log("⚠️ 悬停后仍未找到删除按钮，可能需要调整选择器");

      // 尝试查找所有可能的删除按钮
      const allSvgs = await page.$$("svg");
      console.log(`🔍 页面中共找到 ${allSvgs.length} 个 SVG 元素`);
      ...
      for (let i = 0; i < Math.min(allSvgs.length, 5); i++) {
        try {
          ...
          if (boundingBox) {
            console.log(
              `🔍 SVG ${i}: 位置 (${boundingBox.x}, ${boundingBox.y})`
            );
          }
        } catch (e) {
          // 忽略错误
...
async function uploadReferenceImage(
  ...
  try {
    ...
    if (imgContainerSelector && imgCloseSelector) {
      await removeExistingReferenceImage(
        page,
        imgContainerSelector,
        imgCloseSelector
      );
    ...
    try {
      ...
      console.log(`✅ 找到参考图片: ${imagePath}`);
    } catch (error) {
      console.log(`⚠️ 参考图片不存在: ${imagePath}，跳过上传`);
      return;
    }
