...
async function checkImagesExist(name) {
  ...
  try {
    ...
    console.log(`📁 找到 ${files.length} 个文件，其中 ${imageFiles.length} 个图片文件`);
...
async function runJimengVideoFlow(jimengVideo, processedData, name) {
  ...
  if (generate_section === 1) {
    ...
    shots = processedData.segments
      ...
      .filter(Boolean);
    console.log(
      `📸 第1批：上传图片 ${imageFiles[imageFiles.length - 1]} 到 ${imageFiles[0]}`
    );
    ...
    shots = processedData.segments
      ...
      .filter(Boolean);
    console.log(
      `📸 第2批：上传图片 ${imageFiles[imageFiles.length - 1]} 到 ${imageFiles[0]}`
    );
    console.log(
      `📝 第2批：运镜描述索引 10-${processedData.segments.length - 1} (共 ${shots.length} 个)`
    );
  } else {
    throw new Error(
      `不支持的 generate_section 值: ${generate_section}，只支持 1 或 2`
    );
  }
  ...
  try {
    ...
    if (needsRefresh) {
      console.log("🔄 检测到比例设置已更改，刷新页面以应用设置...");
      await page.reload({ waitUntil: "networkidle2" });
      await page.waitForTimeout(3000);
      console.log("✅ 页面刷新完成");
    } else {
      console.log("✅ 比例设置已正确，无需刷新页面");
    }
