  ...
  for (let i = 0; i < voiceList.length; i++) {
    ...
    try {
      ...
      if (sourceDuration >= targetDuration) {
        ...
        console.log(`🎵 音频时长 ${sourceDuration.toFixed(2)}s < 目标 ${targetDuration}s，只播放一次，不重复`);
 ...
 */
function generateFreezeFrameAnimationFilter(animationType, duration, width, height) {
  const fps = 30; // 帧率
...
async function addFreezeFrames(
  ...
  try {
    ...
    for (let i = 0; i < freezePoints.length; i++) {
      ...
      // 获取当前片段的定格时长（动态或固定）
      const currentFreezeDuration = useDynamicFreeze && i < dynamicFreezeFrames.length
      ...
      try {
        ...
        const actualSegmentDuration = parseFloat(segmentInfo.format?.duration || 0);
        
        if (Math.abs(actualSegmentDuration - segmentDuration) > 0.5) {
          console.warn(`    ⚠️ 片段时长不符: 预期${segmentDuration.toFixed(2)}s, 实际${actualSegmentDuration.toFixed(2)}s`);
        }
      } catch (error) {
    ...
    if (lastDuration > 0.1) {
      ...
      try {
        ...
        if (Math.abs(actualSegmentDuration - lastDuration) > 0.5) {
          console.warn(`    ⚠️ 最后片段时长不符: 预期${lastDuration.toFixed(2)}s, 实际${actualSegmentDuration.toFixed(2)}s`);
        }
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      console.log(`  ✓ 片段${i + 1}: ${path.basename(seg)} (${(stats.size / 1024).toFixed(2)}KB)`);
    }
    ...
    for (let i = 0; i < freezePoints.length; i++) {
      ...
      const currentFreezeDurationCalc = useDynamicFreeze && i < dynamicFreezeFrames.length
        ? dynamicFreezeFrames[i]
        : freezeDuration;
    ...
    try {
      ...
      if (Math.abs(outputDuration - expectedDuration) > 1) {
        console.warn(`⚠️ 输出时长与预期不符！差异: ${Math.abs(outputDuration - expectedDuration).toFixed(2)}s`);
      }
    } catch (error) {
...
async function addSectionTitles(
  ...
  // 调试日志：检查结尾标题参数
  console.log(`[DEBUG] endTitle: "${endTitle}", endTitleDuration: ${endTitleDuration}`);
  ...
  try {
    ...
    if (endTitle) {
      ...
      console.log(`[DEBUG] 结尾标题时间范围: ${endTitleStart}s - ${endTitleEnd}s`);
    ...
    if (finalEndTitleDuration > 0) {
      // 有结尾标题：延长视频并烧录字幕
      console.log(`[3/5] 检测到结尾标题，延长视频时长: ${duration}s -> ${totalVideoDuration}s`);
  ...
  if (useBgMusic) {
    ...
    if (hasOriginalAudio) {
      ...
      if (hasTypewriterSound && hasBabyCry) {
        ...
        execCommand(
          `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" -i "${babyCryInfo.path}" -filter_complex "${videoFilter};[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_WITH_BG}[a0];[1:a]volume=${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME},apad=pad_dur=${videoDuration}[a1];[2:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=2[a2];[a0][a1][a2]amix=inputs=3:duration=longest[aout]" -map "[v]" -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
          true // 显示进度
        );
      } else {
        // 检查是否有片段配音
    ...
    if (hasTypewriterSound || hasBabyCry) {
      ...
      if (hasOriginalAudio) {
        if (hasTypewriterSound && hasBabyCry) {
          ...
          execCommand(
            `ffmpeg -y -i "${videoPath}" -i "${typewriterSound.path}" -i "${babyCryInfo.path}" -filter_complex "${videoFilter};[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_SOLO}[a0];[1:a]adelay=${typewriterSound.start * 1000}|${typewriterSound.start * 1000},volume=${CONFIG_AUDIO_VOLUME.TYPEWRITER_SOUND_VOLUME}[a1];[2:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=2[a2];[a0][a1][a2]amix=inputs=3:duration=longest[aout]" -map "[v]" -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        } else if (hasTypewriterSound) {
          // 原视频有音频，混合原音频和打字机音效
...
export default async function runHistoryPerson(config) {
  ...
  if (freezeFrame && freezeFrame > 0) {
    const animation = freezeFrameAnimation || CONFIG_FREEZE_FRAME_ANIMATION.DEFAULT;
        ...
        if (mp3Files.length > 0) {
          ...
          voiceList = voicesWithDuration.map((item, index) => {
            ...
            if (baseFreezeFrame > 0) {
              // 有定格帧模式
              const totalAvailableTime = normalPlayDuration + baseFreezeDuration;
          ...
          if (baseFreezeFrame > 0) {
            ...
            console.log(`✅ 成功生成 ${voiceList.length} 个配音片段 (动态定格帧):`);
          } else {
            console.log(`✅ 成功生成 ${voiceList.length} 个配音片段:`);
          }
          
          voiceList.forEach((voice, index) => {
            const freezeInfo = voice.freezeDuration > 0 
              ? ` (配音${voice.voiceDuration.toFixed(2)}s, 定格${voice.freezeDuration.toFixed(2)}s)`
              : '';
            console.log(`   ${index}: ${voice.duration[0].toFixed(1)}s-${voice.duration[1].toFixed(1)}s -> ${voice.voice}${freezeInfo}`);
          });
  ...
  try {
    ...
    if (freezeFrame && freezeFrame > 0) {
      ...
      const dynamicFreezeFrames = global.dynamicFreezeFrames || [];
      console.log(`[DEBUG] dynamicFreezeFrames 数量: ${dynamicFreezeFrames.length}`);
      if (dynamicFreezeFrames.length > 0) {
        console.log(`[DEBUG] 前3个定格时长: ${dynamicFreezeFrames.slice(0, 3).map(d => d.toFixed(2)).join(', ')}`);
      }
