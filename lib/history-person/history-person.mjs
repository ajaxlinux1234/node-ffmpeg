  ...
  if (useBgMusic) {
    ...
    if (hasOriginalAudio) {
      ...
      if (hasTypewriterSound && hasBabyCry) {
        ...
        execCommand(
          `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" -i "${typewriterSound.path}" -i "${babyCryInfo.path}" -filter_complex "[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_WITH_BG}[a0];[1:a]volume=${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME},apad=pad_dur=${videoDuration}[a1];[2:a]adelay=${typewriterSound.start * 1000}|${typewriterSound.start * 1000},volume=${CONFIG_AUDIO_VOLUME.TYPEWRITER_SOUND_VOLUME}[a2];[3:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=2[a3];[a0][a1][a2][a3]amix=inputs=4:duration=longest[aout]" -map 0:v -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
          true // 显示进度
        );
        ...
        execCommand(
          `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" -i "${typewriterSound.path}" -filter_complex "[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_WITH_BG}[a0];[1:a]volume=${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME},apad=pad_dur=${videoDuration}[a1];[2:a]adelay=${typewriterSound.start * 1000}|${typewriterSound.start * 1000},volume=${CONFIG_AUDIO_VOLUME.TYPEWRITER_SOUND_VOLUME}[a2];[a0][a1][a2]amix=inputs=3:duration=longest[aout]" -map 0:v -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
          true // 显示进度
        );
      } else if (hasBabyCry) {
        // 混合原视频音频、背景音乐和婴儿哭声
        ...
        execCommand(
          `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" -i "${babyCryInfo.path}" -filter_complex "[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_WITH_BG}[a0];[1:a]volume=${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME},apad=pad_dur=${videoDuration}[a1];[2:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=2[a2];[a0][a1][a2]amix=inputs=3:duration=longest[aout]" -map 0:v -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
          true // 显示进度
        );
      } else {
        // 检查是否有片段配音
        ...
        if (processedVoices.length > 0) {
          ...
          execCommand(
            `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" ${processedVoices.map((voice) => `-i "${voice.file}"`).join(" ")} -filter_complex "${filterComplex}" -map 0:v -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        } else {
          // 混合原视频音频和背景音乐
          ...
          execCommand(
            `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" -filter_complex "[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_WITH_BG}[a0];[1:a]volume=${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME},apad=pad_dur=${videoDuration}[a1];[a0][a1]amix=inputs=2:duration=longest[aout]" -map 0:v -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        }
      }
      ...
      if (hasTypewriterSound && hasBabyCry) {
        ...
        execCommand(
          `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" -i "${typewriterSound.path}" -i "${babyCryInfo.path}" -filter_complex "[1:a]volume=${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME},apad=pad_dur=${videoDuration}[a1];[2:a]adelay=${typewriterSound.start * 1000}|${typewriterSound.start * 1000},volume=${CONFIG_AUDIO_VOLUME.TYPEWRITER_SOUND_VOLUME}[a2];[3:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=2[a3];[a1][a2][a3]amix=inputs=3:duration=longest[aout]" -map 0:v -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
          true // 显示进度
        );
      } else if (hasTypewriterSound) {
        // 混合背景音乐和打字机音效
        ...
        execCommand(
          `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" -i "${typewriterSound.path}" -filter_complex "[1:a]volume=${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME},apad=pad_dur=${videoDuration}[a1];[2:a]adelay=${typewriterSound.start * 1000}|${typewriterSound.start * 1000},volume=${CONFIG_AUDIO_VOLUME.TYPEWRITER_SOUND_VOLUME}[a2];[a1][a2]amix=inputs=2:duration=longest[aout]" -map 0:v -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
          true // 显示进度
        );
      } else if (hasBabyCry) {
        // 混合背景音乐和婴儿哭声
        ...
        execCommand(
          `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" -i "${babyCryInfo.path}" -filter_complex "[1:a]volume=${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME},apad=pad_dur=${videoDuration}[a1];[2:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=2[a2];[a1][a2]amix=inputs=2:duration=longest[aout]" -map 0:v -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
          true // 显示进度
        );
      } else {
        // 检查是否有片段配音
    ...
    if (hasTypewriterSound || hasBabyCry) {
      ...
      if (hasOriginalAudio) {
        if (hasTypewriterSound && hasBabyCry) {
          ...
          execCommand(
            `ffmpeg -y -i "${videoPath}" -i "${typewriterSound.path}" -i "${babyCryInfo.path}" -filter_complex "[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_SOLO}[a0];[1:a]adelay=${typewriterSound.start * 1000}|${typewriterSound.start * 1000},volume=${CONFIG_AUDIO_VOLUME.TYPEWRITER_SOUND_VOLUME}[a1];[2:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=2[a2];[a0][a1][a2]amix=inputs=3:duration=longest[aout]" -map 0:v -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        } else if (hasTypewriterSound) {
          // 原视频有音频，混合原音频和打字机音效
          execCommand(
            `ffmpeg -y -i "${videoPath}" -i "${typewriterSound.path}" -filter_complex "[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_SOLO}[a0];[1:a]adelay=${typewriterSound.start * 1000}|${typewriterSound.start * 1000},volume=${CONFIG_AUDIO_VOLUME.TYPEWRITER_SOUND_VOLUME}[a1];[a0][a1]amix=inputs=2:duration=longest[aout]" -map 0:v -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        } else if (hasBabyCry) {
          // 原视频有音频，混合原音频和婴儿哭声
          execCommand(
            `ffmpeg -y -i "${videoPath}" -i "${babyCryInfo.path}" -filter_complex "[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_SOLO}[a0];[1:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=2[a1];[a0][a1]amix=inputs=2:duration=longest[aout]" -map 0:v -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        }
      } else {
        if (hasTypewriterSound && hasBabyCry) {
          ...
          // 原视频无音频，只添加打字机音效
          execCommand(
            `ffmpeg -y -i "${videoPath}" -i "${typewriterSound.path}" -filter_complex "[1:a]adelay=${typewriterSound.start * 1000}|${typewriterSound.start * 1000},volume=${CONFIG_AUDIO_VOLUME.TYPEWRITER_SOUND_VOLUME},apad=pad_dur=${videoDuration}[aout]" -map 0:v -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        } else if (hasBabyCry) {
          // 原视频无音频，只添加婴儿哭声
          execCommand(
            `ffmpeg -y -i "${videoPath}" -i "${babyCryInfo.path}" -filter_complex "[1:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=${videoDuration}[aout]" -map 0:v -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        }
      }
