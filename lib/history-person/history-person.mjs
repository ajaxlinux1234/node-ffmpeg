...
async function processVoiceClips(voiceList, outputDir, videoDuration) {
  ...
  for (let i = 0; i < voiceList.length; i++) {
    ...
    // 计算需要重复的次数
    const repeatCount = Math.ceil(targetDuration / sourceDuration);
    console.log(
      `🔄 重复播放: ${repeatCount}次 (实际时长: ${(sourceDuration * repeatCount).toFixed(2)}s)`
    );
    ...
    const processedFile = path.join(
      voicesDir,
      `${voiceName}_${startTime}s-${endTime}s_${hash}.wav`
    );

    try {
      if (repeatCount > 1) {
        // 需要循环播放
        const concatList = Array(repeatCount)
          .fill(`file '${path.resolve(voiceFile)}'`)
          .join("\n");
        const concatFile = path.join(voicesDir, `concat_${hash}.txt`);
        await fs.writeFile(concatFile, concatList);

        execCommand(
          `ffmpeg -y -f concat -safe 0 -i "${concatFile}" -t ${targetDuration} -c:a pcm_s16le -ar 44100 "${processedFile}"`
        );
