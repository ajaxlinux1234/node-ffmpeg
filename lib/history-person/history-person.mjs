...
import { showOptimizationInfo } from "../ffmpeg-optimization.mjs";
import {
  CONFIG_SUBTITLE,
  CONFIG_SPACING,
  CONFIG_WATERMARK,
 ...
 * @param {boolean} showProgress - 是否显示进度信息
 * @returns {Object} - {stdout: string, stderr: string}
 */
function execCommand(command, showProgress = false) {
  try {
    // 如果需要显示进度，添加FFmpeg进度参数
    if (showProgress && command.includes("ffmpeg")) {
      // 添加进度显示参数
      command = command.replace(
        "ffmpeg -y",
        "ffmpeg -y -progress pipe:1 -stats"
      );
    }

    const stdout = execSync(command, {
      encoding: "utf8",
      stdio: showProgress ? "inherit" : "pipe", // 显示进度时使用inherit来实时输出
    });
 ...
 */
function convertCSSToFFmpegStyle(style, font) {
  const result = {
    fontname: font || getPlatformFont(),
    fontsize: CONFIG_SUBTITLE.FONT_SIZE_TITLE,
    primaryColour: "&H0000FFFF", // 默认黄色
    bold: 1,
  };
...
/**
 * 生成带字体配置的ASS样式
 * @param {Object} fontStyle - 转换后的字体样式
 * @param {string} styleName - 样式名称
 * @param {number} W - 视频宽度
 * @param {number} H - 视频高度
 * @returns {string} - ASS样式字符串
 */
function generateASSStyleWithFont(fontStyle, styleName, W, H) {
  const bottomMargin = Math.floor(
    H * CONFIG_SUBTITLE.SUBTITLE_POSITION_Y_PERCENT
  );
  const charSpacing = 0;

  return `Style: ${styleName}, ${fontStyle.fontname}, ${fontStyle.fontsize}, ${fontStyle.primaryColour}, ${fontStyle.primaryColour}, &H80000000, &H80000000, ${fontStyle.bold}, 0, 0, 0, 100, 100, ${charSpacing}, 0, 1, 3, 2, 2, ${CONFIG_SUBTITLE.LEFT_MARGIN}, ${CONFIG_SUBTITLE.RIGHT_MARGIN}, ${bottomMargin}, 0\n`;
}
...
/**
 * 处理片段配音
 * @param {Array} voiceList - 配音列表
 * @param {string} outputDir - 输出目录
 * @param {number} videoDuration - 视频时长
 * @returns {Promise<Array>} - 处理后的配音文件路径数组
 */
async function processVoiceClips(voiceList, outputDir, videoDuration) {
  if (!voiceList || voiceList.length === 0) {
    return [];
  }

  console.log(`[3/5] 准备处理 ${voiceList.length} 个片段配音...`);

  const processedVoices = [];
  const voicesDir = path.join(outputDir, "voices");
  await fs.mkdir(voicesDir, { recursive: true });

  for (let i = 0; i < voiceList.length; i++) {
    const voice = voiceList[i];
    console.log(`🔍 处理配音 ${i + 1}: ${JSON.stringify(voice)}`);

    const { duration, voice: voiceName } = voice;

    if (!Array.isArray(duration) || duration.length !== 2) {
      console.warn(
        `⚠️ 配音 "${voiceName}" duration格式错误，应为[开始时间, 结束时间]: ${JSON.stringify(duration)}`
      );
      continue;
    }
    ...
    if (targetDuration <= 0) {
      console.warn(
        `⚠️ 配音 "${voiceName}" 时间区间无效: ${startTime}s-${endTime}s`
      );
      continue;
    }

    // 查找配音文件
    const voiceFile = await findVoiceFile(voiceName);
    if (!voiceFile) {
      console.warn(`⚠️ 未找到配音文件: ${voiceName}`);
      continue;
    }

    console.log(`✅ 找到配音文件: ${voiceFile}`);

    // 获取源音频时长
    let sourceDuration = 0;
    try {
      const probe = execCommand(
        `ffprobe -v quiet -print_format json -show_format "${voiceFile}"`
      );
      const info = JSON.parse(probe.stdout);
      sourceDuration = parseFloat(info.format?.duration || 0);
    } catch (error) {
      console.warn(`⚠️ 无法获取配音文件时长: ${voiceFile}`);
      continue;
    }

    console.log(
      `📊 配音信息: ${sourceDuration.toFixed(2)}s (源) -> ${targetDuration}s (目标)`
    );

    // 生成处理后的配音文件
    const hash = crypto
      .createHash("md5")
      .update(`${voiceName}_${startTime}_${endTime}`)
      .digest("hex")
      .substring(0, 8);
    // 从 voiceName 中提取文件名（去除路径部分）
    const voiceBaseName = path.basename(voiceName);
    const processedFile = path.join(
      voicesDir,
      `${voiceBaseName}_${startTime}s-${endTime}s_${hash}.wav`
    );

    try {
      // 判断是否需要裁剪或填充
      if (sourceDuration >= targetDuration) {
        // 音频时长大于等于目标时长，直接裁剪到目标时长
        console.log(`✂️ 音频时长足够，裁剪到 ${targetDuration}s`);
        execCommand(
          `ffmpeg -y -i "${voiceFile}" -t ${targetDuration} -c:a pcm_s16le -ar 44100 "${processedFile}"`
        );
      } else {
        // 音频时长小于目标时长，只播放一次，不重复
        console.log(
          `🎵 音频时长 ${sourceDuration.toFixed(2)}s < 目标 ${targetDuration}s，只播放一次，不重复`
        );

        // 直接复制音频，不裁剪，保持原始时长
        execCommand(
          `ffmpeg -y -i "${voiceFile}" -c:a pcm_s16le -ar 44100 "${processedFile}"`
        );
      }
      ...
      console.log(`✅ 配音处理完成: ${voiceName} (${startTime}s-${endTime}s)`);
    } catch (error) {
      console.warn(`⚠️ 配音处理失败: ${voiceName} - ${error.message}`);
    }
  }

  console.log(`📁 配音处理完成，共生成 ${processedVoices.length} 个配音文件`);
  return processedVoices;
}

/**
 * 查找配音文件
 * @param {string} voiceName - 配音名称（可以是相对路径，如 "刘强东/lines/0"）
 * @returns {Promise<string|null>} - 配音文件路径
 */
async function findVoiceFile(voiceName) {
  const extensions = [".mp3", ".wav", ".m4a"];

  // 如果 voiceName 包含路径分隔符，说明是相对路径
  if (voiceName.includes("/") || voiceName.includes("\\")) {
    // 尝试从 output 目录查找
    for (const ext of extensions) {
      const filePath = path.join(process.cwd(), "output", voiceName + ext);
      try {
  ...
  // 默认查找逻辑：在 sounds 和 voices 目录下查找
  const directories = ["sounds", "voices"];

  for (const dir of directories) {
    for (const ext of extensions) {
      const filePath = path.join(process.cwd(), dir, voiceName + ext);
      try {
        if (await fs.pathExists(filePath)) {
          return filePath;
        }
 ...
 */
function generateFreezeFrameAnimationFilter(
  animationType,
  duration,
  width,
  height
  ...
  switch (animationType) {
    case "zoom_in":
      // 从小到大放大（从80%到100%）
      return `zoompan=z='min(zoom+0.0015,1.25)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${totalFrames}:s=${width}x${height}:fps=${fps}`;

    case "zoom_out":
      // 从大到小缩小（从120%到100%）
      return `zoompan=z='if(lte(zoom,1.0),1.0,max(1.0,zoom-0.002))':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${totalFrames}:s=${width}x${height}:fps=${fps}`;
    ...
    case "slide_up":
      // 从下往上滑入
      return `crop=iw:ih:0:'max(0,ih-ih*t/${duration})'`;

    case "slide_down":
      // 从上往下滑入
      return `crop=iw:ih:0:'min(0,ih*t/${duration}-ih)'`;

    case "pulse":
      // 脉冲效果（放大缩小循环）
      return `zoompan=z='1.0+0.1*sin(2*PI*t/${duration})':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${totalFrames}:s=${width}x${height}:fps=${fps}`;

    case "rotate":
      // 旋转效果（360度旋转）
      return `rotate=a='2*PI*t/${duration}':c=none:ow=${width}:oh=${height}`;

    case "shake":
      // 震动效果
      return `crop=iw:ih:'5*sin(20*2*PI*t)':'5*cos(20*2*PI*t)'`;

    case "blur_in":
      // 从模糊到清晰
      return `gblur=sigma='max(0,10-10*t/${duration})'`;
...
/**
 * 为视频添加定格帧效果（每5秒最后一帧定格指定时长）
 * @param {string} videoPath - 输入视频路径
 * @param {string} outputDir - 输出目录
 * @param {number} freezeDuration - 基础定格时长（秒）
 * @param {number} interval - 间隔时长（秒），默认5秒
 * @param {string} qualityMode - 质量模式
 * @param {boolean} enableSpeedOptimization - 是否启用速度优化
 * @param {string} animationType - 动画类型
 * @returns {Promise<string>} - 输出视频路径
 */
async function addFreezeFrames(
  videoPath,
  outputDir,
  freezeDuration,
  interval = 5,
  ...
  // 检查是否有动态定格帧配置
  const dynamicFreezeFrames = global.dynamicFreezeFrames || [];
  const useDynamicFreeze = dynamicFreezeFrames.length > 0;

  if (useDynamicFreeze) {
    console.log(`[2.5/5] 正在添加动态定格帧效果（根据配音时长自动调整）...`);
  } else {
    console.log(
      `[2.5/5] 正在添加定格帧效果（每${interval}秒定格${freezeDuration}秒）...`
    );
  }
  ...
  const videoWidth = videoStream?.width || CONFIG_VIDEO.DEFAULT_WIDTH;
  const videoHeight = videoStream?.height || CONFIG_VIDEO.DEFAULT_HEIGHT;

  if (videoDuration <= 0) {
    console.warn(`⚠️ 无法获取视频时长，跳过定格帧处理`);
    return videoPath;
  }

  console.log(`📊 视频总时长: ${videoDuration.toFixed(2)}s`);
  ...
  if (freezePoints.length === 0) {
    console.log(`⚠️ 视频时长不足${interval}秒，无需添加定格帧`);
    return videoPath;
  }

  if (useDynamicFreeze) {
    console.log(
      `📍 定格点: ${freezePoints.length}个，每个定格时长根据配音动态调整`
    );
  } else {
    console.log(
      `📍 定格点: ${freezePoints.map((t) => `${t}s`).join(", ")} (共${freezePoints.length}个)`
    );
  }

  // 创建临时目录
  const tempDir = path.join(outputDir, "freeze_temp");
  await fs.mkdir(tempDir, { recursive: true });

  try {
    // 分割视频片段并提取定格帧
    const segments = [];
    let currentTime = 0; // 当前累计时间（包括定格帧）

    for (let i = 0; i < freezePoints.length; i++) {
      const freezeTime = freezePoints[i]; // 原视频中的定格点位置
      const segmentPath = path.join(tempDir, `segment_${i}.mp4`);
      const framePath = path.join(tempDir, `frame_${i}.png`);
      const freezeVideoPath = path.join(tempDir, `freeze_${i}.mp4`);

      // 获取当前片段的定格时长（动态或固定）
      const currentFreezeDuration =
        useDynamicFreeze && i < dynamicFreezeFrames.length
          ? dynamicFreezeFrames[i]
          : freezeDuration;

      // 1. 提取片段（从上一个定格点到当前定格点，在原视频中的位置）
      const prevFreezeTime = i === 0 ? 0 : freezePoints[i - 1];
      const segmentDuration = freezeTime - prevFreezeTime;

      if (useDynamicFreeze) {
        console.log(
          `  📹 片段${i + 1}: ${prevFreezeTime.toFixed(2)}s - ${freezeTime.toFixed(2)}s (${segmentDuration.toFixed(2)}s) + 定格${currentFreezeDuration.toFixed(2)}s`
        );
      } else {
        console.log(
          `  📹 片段${i + 1}: ${prevFreezeTime.toFixed(2)}s - ${freezeTime.toFixed(2)}s (${segmentDuration.toFixed(2)}s)`
        );
      }

      try {
        // 从原视频中提取片段 - 统一使用编码模式以确保准确性
        const encodingParams = getOptimizedEncodingParams(
          qualityMode || "high",
          enableSpeedOptimization || false
        );
        ...
        // 使用精确的时间定位：先seek再读取
        execCommand(
          `ffmpeg -y -ss ${prevFreezeTime} -i "${videoPath}" -t ${segmentDuration} ${encodingParams} -avoid_negative_ts 1 "${segmentPath}"`
        );

        // 验证片段是否生成成功
        if (!(await fs.pathExists(segmentPath))) {
          throw new Error(`片段 ${segmentPath} 生成失败`);
        }

        // 验证片段时长
        const segmentProbe = execCommand(
          `ffprobe -v quiet -print_format json -show_format "${segmentPath}"`
        );
        const segmentInfo = JSON.parse(segmentProbe.stdout);
        const actualSegmentDuration = parseFloat(
          segmentInfo.format?.duration || 0
        );

        if (Math.abs(actualSegmentDuration - segmentDuration) > 0.5) {
          console.warn(
            `    ⚠️ 片段时长不符: 预期${segmentDuration.toFixed(2)}s, 实际${actualSegmentDuration.toFixed(2)}s`
          );
        }
      } catch (error) {
        console.error(`❌ 提取片段${i + 1}失败: ${error.message}`);
        throw error;
      }
      ...
      try {
        execCommand(
          `ffmpeg -y -ss ${freezeTime - 0.04} -i "${videoPath}" -vframes 1 "${framePath}"`
        );

        // 验证帧是否提取成功
        if (!(await fs.pathExists(framePath))) {
          throw new Error(`定格帧 ${framePath} 提取失败`);
        }
      } catch (error) {
        console.error(`❌ 提取定格帧${i + 1}失败: ${error.message}`);
        throw error;
      }

      // 3. 将定格帧转换为视频（持续动态或固定时长，添加动画效果）
      const encodingParams = getOptimizedEncodingParams(
        qualityMode || "high",
        enableSpeedOptimization || false
      );
      ...
      // 生成动画滤镜
      const animationFilter = generateFreezeFrameAnimationFilter(
        animationType,
      ...
      try {
        if (animationFilter) {
          // 有动画效果
          execCommand(
            `ffmpeg -y -loop 1 -i "${framePath}" -vf "${animationFilter}" -t ${currentFreezeDuration} ${encodingParams} -pix_fmt yuv420p "${freezeVideoPath}"`
          );
        } else {
          // 无动画效果
          execCommand(
            `ffmpeg -y -loop 1 -i "${framePath}" -t ${currentFreezeDuration} ${encodingParams} -pix_fmt yuv420p "${freezeVideoPath}"`
          );
        }

        // 验证定格视频是否生成成功
        if (!(await fs.pathExists(freezeVideoPath))) {
          throw new Error(`定格视频 ${freezeVideoPath} 生成失败`);
        }
      } catch (error) {
        console.error(`❌ 生成定格视频${i + 1}失败: ${error.message}`);
        throw error;
      }

      segments.push(segmentPath);
      segments.push(freezeVideoPath);

      currentTime += segmentDuration + currentFreezeDuration;
    }

    // 添加最后一个片段（从最后一个定格点到视频结束）
    const lastSegmentPath = path.join(
      tempDir,
      `segment_${freezePoints.length}.mp4`
    );
    const lastFreezeTime = freezePoints[freezePoints.length - 1];
    const lastDuration = videoDuration - lastFreezeTime;
    if (lastDuration > 0.1) {
      console.log(
        `  📹 最后片段: ${lastFreezeTime.toFixed(2)}s - ${videoDuration.toFixed(2)}s (${lastDuration.toFixed(2)}s)`
      );
      try {
        // 提取最后片段 - 使用编码模式
        const encodingParams = getOptimizedEncodingParams(
        ...
        execCommand(
          `ffmpeg -y -ss ${lastFreezeTime} -i "${videoPath}" -t ${lastDuration} ${encodingParams} -avoid_negative_ts 1 "${lastSegmentPath}"`
        );

        // 验证最后片段是否生成成功
        if (!(await fs.pathExists(lastSegmentPath))) {
          throw new Error(`最后片段 ${lastSegmentPath} 生成失败`);
        }

        // 验证片段时长
        const segmentProbe = execCommand(
          `ffprobe -v quiet -print_format json -show_format "${lastSegmentPath}"`
        );
        const segmentInfo = JSON.parse(segmentProbe.stdout);
        const actualSegmentDuration = parseFloat(
          segmentInfo.format?.duration || 0
        ...
        if (Math.abs(actualSegmentDuration - lastDuration) > 0.5) {
          console.warn(
            `    ⚠️ 最后片段时长不符: 预期${lastDuration.toFixed(2)}s, 实际${actualSegmentDuration.toFixed(2)}s`
          );
        }

        segments.push(lastSegmentPath);
      } catch (error) {
        console.error(`❌ 提取最后片段失败: ${error.message}`);
        throw error;
      }
    }

    // 验证所有片段是否都存在
    console.log(`🔍 验证 ${segments.length} 个片段...`);
    for (let i = 0; i < segments.length; i++) {
      const seg = segments[i];
      if (!(await fs.pathExists(seg))) {
        throw new Error(`片段 ${i + 1} 不存在: ${seg}`);
      }

      // 检查文件大小
      const stats = await fs.stat(seg);
      if (stats.size === 0) {
        throw new Error(`片段 ${i + 1} 文件大小为0: ${seg}`);
      }
      console.log(
        `  ✓ 片段${i + 1}: ${path.basename(seg)} (${(stats.size / 1024).toFixed(2)}KB)`
      );
    }
    ...
    // 5. 拼接所有片段
    const base = path.basename(videoPath, path.extname(videoPath));
    const outputPath = path.join(outputDir, `${base}_freeze.mp4`);

    console.log(`🔗 正在拼接 ${segments.length} 个片段...`);

    // 计算预期的总时长
    let expectedDuration = 0;
    for (let i = 0; i < freezePoints.length; i++) {
      const prevFreezeTimeCalc = i === 0 ? 0 : freezePoints[i - 1];
      const freezeTimeCalc = freezePoints[i];
      const segmentDurationCalc = freezeTimeCalc - prevFreezeTimeCalc;
      const currentFreezeDurationCalc =
        useDynamicFreeze && i < dynamicFreezeFrames.length
          ? dynamicFreezeFrames[i]
          : freezeDuration;
      expectedDuration += segmentDurationCalc + currentFreezeDurationCalc;
    }
    const lastFreezeTimeCalc = freezePoints[freezePoints.length - 1];
    const lastDurationCalc = videoDuration - lastFreezeTimeCalc;
    if (lastDurationCalc > 0.1) {
      expectedDuration += lastDurationCalc;
    }

    console.log(`📊 原视频时长: ${videoDuration.toFixed(2)}s`);
    console.log(`📊 预期输出时长: ${expectedDuration.toFixed(2)}s`);
    ...
    execCommand(
      `ffmpeg -y -f concat -safe 0 -i "${concatListPath}" ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_COPY} "${outputPath}"`
    );

    // 验证输出视频时长
    try {
      const outputProbe = execCommand(
        `ffprobe -v quiet -print_format json -show_format "${outputPath}"`
      );
      const outputInfo = JSON.parse(outputProbe.stdout);
      const outputDuration = parseFloat(outputInfo.format?.duration || 0);
      console.log(`📊 实际输出时长: ${outputDuration.toFixed(2)}s`);

      if (Math.abs(outputDuration - expectedDuration) > 1) {
        console.warn(
          `⚠️ 输出时长与预期不符！差异: ${Math.abs(outputDuration - expectedDuration).toFixed(2)}s`
        );
      }
    } catch (error) {
      console.warn(`⚠️ 无法验证输出视频时长: ${error.message}`);
    }

    // 清理临时文件
    await fs.remove(tempDir).catch(() => {});

    console.log(`✅ 定格帧效果添加完成: ${outputPath}`);
    return outputPath;
  } catch (error) {
    console.error(`❌ 定格帧处理失败: ${error.message}`);
    // 清理临时文件
    await fs.remove(tempDir).catch(() => {});
    return videoPath;
  }
...
async function addBlurMask(videoPath, outputDir, opt) {
  ...
  // 如果 w 和 h 都为 0，跳过模糊遮罩处理，直接返回原视频
  if (w === 0 && h === 0) {
    console.log(`⏭️  跳过左上角水印遮罩 (w=0, h=0)`);
    return videoPath;
  }
 ...
 * @param {number} endTitleDuration - 结尾标题显示时长（秒）
 * @param {string} endTitleAnimation - 结尾标题动画效果
 * @param {string} endTitleSound - 结尾标题声音效果
 * @param {boolean} enableSpeedOptimization - 是否启用速度优化
...
async function addSectionTitles(
  ...
  sectionTitleInterval, // 新增：分镜字幕间隔时间
  sectionLastTitleOffset, // 新增：最后一个分镜提前显示时间
  voiceList, // 片段配音列表
  useLines, // 新增：是否使用 lines 模式
  titleLineBreak = "\n", // history-person 标题换行符
  globalFontConfig = null, // 全局字体配置
  segmentFontConfigs = null, // 分镜字体配置数组
  titleFontConfig = null // 全局标题字体配置
) {
  // 调试日志：检查结尾标题参数
  console.log(
    `[DEBUG] endTitle: "${endTitle}", endTitleDuration: ${endTitleDuration}`
  );
  ...
  if (
    ...
    console.log(`[3/5] 没有任何字幕内容，跳过字幕生成步骤`);
    return videoPath;
  }
  ...
  try {
    ...
    // 生成动态字体样式（如果有字体配置）
    const dynamicStyles = new Map(); // 存储已生成的样式，避免重复

    // 处理全局标题字体配置
    if (titleFontConfig && Array.isArray(titleFontConfig)) {
      titleFontConfig.forEach((config, index) => {
        const fontStyle = convertCSSToFFmpegStyle(config.style, config.font);
        const styleName = `TitleFont${index}`;
        if (!dynamicStyles.has(styleName)) {
          // 为全局标题生成专用样式（顶部居中对齐）
          const titleBottomMargin = Math.floor(
            H * CONFIG_SPACING.GLOBAL_TITLE_POSITION_Y_PERCENT
          );
          const titleStyle = `Style: ${styleName}, ${fontStyle.fontname}, ${fontStyle.fontsize}, ${fontStyle.primaryColour}, ${fontStyle.primaryColour}, &H00000000, &H80000000, ${fontStyle.bold}, 0, 0, 0, 100, 100, 0, 0, 1, 4, 0, 8, ${CONFIG_SPACING.GLOBAL_TITLE_MARGIN_LEFT}, ${CONFIG_SPACING.GLOBAL_TITLE_MARGIN_RIGHT}, ${titleBottomMargin}, 0\n`;
          ass += titleStyle;
          dynamicStyles.set(styleName, fontStyle);
        }
      });
    }

    // 处理全局字体配置
    if (globalFontConfig && Array.isArray(globalFontConfig)) {
      globalFontConfig.forEach((config, index) => {
        const fontStyle = convertCSSToFFmpegStyle(config.style, config.font);
        const styleName = `GlobalFont${index}`;
        if (!dynamicStyles.has(styleName)) {
          ass += generateASSStyleWithFont(fontStyle, styleName, W, H);
          dynamicStyles.set(styleName, fontStyle);
        }
      });
    }
    ...
    // 处理分镜字体配置
    if (segmentFontConfigs && Array.isArray(segmentFontConfigs)) {
      segmentFontConfigs.forEach((segmentConfig, segmentIndex) => {
        if (
          segmentConfig &&
          segmentConfig.fontConfig &&
          Array.isArray(segmentConfig.fontConfig)
        ) {
          segmentConfig.fontConfig.forEach((config, lineIndex) => {
            const fontStyle = convertCSSToFFmpegStyle(
              config.style,
              config.font
            );
            const styleName = `SegmentFont${segmentIndex}Line${lineIndex}`;
            if (!dynamicStyles.has(styleName)) {
              ass += generateASSStyleWithFont(fontStyle, styleName, W, H);
              dynamicStyles.set(styleName, fontStyle);
            }
    ...
    // Alignment=8 表示顶部居中对齐，实现水平居中效果
    // 使用负间距来紧密显示数字和缩写词
    ass += `Style: TopTitle, ${platformFont}, ${CONFIG_SUBTITLE.FONT_SIZE_GLOBAL_TITLE}, &H80FFFFFF, &H80FFFFFF, &H00000000, &H80000000, 1, 0, 0, 0, 100, 100, -3, 0, 1, 4, 0, 8, ${CONFIG_SPACING.GLOBAL_TITLE_MARGIN_LEFT}, ${CONFIG_SPACING.GLOBAL_TITLE_MARGIN_RIGHT}, 0, 0\n`;
    // 顶部标题关键词样式：红色填充，黑色描边，其他参数与TopTitle相同
    ass += `Style: TopTitleKeyword, ${platformFont}, ${CONFIG_SUBTITLE.FONT_SIZE_GLOBAL_TITLE}, &H000000FF, &H000000FF, &H00000000, &H80000000, 1, 0, 0, 0, 100, 100, -3, 0, 1, 4, 0, 8, ${CONFIG_SPACING.GLOBAL_TITLE_MARGIN_LEFT}, ${CONFIG_SPACING.GLOBAL_TITLE_MARGIN_RIGHT}, 0, 0\n\n`;
    ...
    const wrapCJK = (s, customLineBreak = "\n") => {
      const inputText = String(s).trim();

      // 检查是否包含手动换行符（支持自定义换行符）
      if (inputText.includes(customLineBreak)) {
        // 手动换行：按自定义换行符分割
        const manualLines = inputText.split(customLineBreak);
    ...
    // 获取配音列表信息（如果使用 useLines 模式）
    const voiceListInfo = global.voiceListInfo || [];
    const useLinesMode = useLines && voiceListInfo.length > 0;

    for (let i = 0; i < sectionTitles.length; i++) {
      let start;
      let titleDuration;

      if (useLinesMode && i < voiceListInfo.length) {
        // useLines 模式：字幕在配音开始时显示
        const voiceInfo = voiceListInfo[i];
        start = voiceInfo.startTime;
        titleDuration = voiceInfo.endTime - voiceInfo.startTime;

        if (i === 0) {
          console.log(`[3/5] useLines模式 - 字幕与配音同步显示`);
        }
      } else if (i === 0 && sectionFirstTitleTime) {
        // 第一个字幕使用特殊时长（非 useLines 模式）
        start = 0;
        titleDuration = sectionFirstTitleTime;
        console.log(
          `[3/5] 第一个字幕特殊处理: 显示时长 ${sectionFirstTitleTime}s`
        );
      } else if (i === sectionTitles.length - 1 && !useLinesMode) {
        // 最后一个字幕的特殊处理（非 useLines 模式）
        const lastTitleOffset = CONFIG_SUBTITLE.SECTION_LAST_TITLE_OFFSET || sectionLastTitleOffset;
        start = duration - lastTitleOffset;
        titleDuration = CONFIG_SUBTITLE.SECTION_TITLE_DURATION;
      } else if (!useLinesMode) {
        // 其他字幕按间隔显示（非 useLines 模式）
        const firstTitleOffset =
          sectionFirstTitleTime || CONFIG_SUBTITLE.SECTION_TITLE_DURATION;
        const titleInterval = sectionTitleInterval || CONFIG_SUBTITLE.SECTION_TITLE_INTERVAL;
        ...
        if (i === 1 && sectionFirstTitleTime) {
          ...
          start = firstTitleOffset + adjustedIndex * titleInterval;
        }
        titleDuration = CONFIG_SUBTITLE.SECTION_TITLE_DURATION;
      }

      const end = Math.min(start + titleDuration - 1, duration); // 确保不超出视频时长
      ...
      const currentSegmentConfig =
        segmentFontConfigs && segmentFontConfigs[i]
          ? segmentFontConfigs[i]
          : null;
      const currentFontConfig =
        currentSegmentConfig && currentSegmentConfig.fontConfig
          ? currentSegmentConfig.fontConfig
          : null;
      ...
      for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        ...
        // 确定当前行使用的字体配置
        let lineConfig = null;
        if (currentFontConfig && Array.isArray(currentFontConfig)) {
          // 如果当前分镜有字体配置，使用对应行的配置
          if (lineIndex < currentFontConfig.length) {
            lineConfig = currentFontConfig[lineIndex];
          } else if (currentFontConfig.length > 0) {
            // 如果行数超过配置数量，使用最后一个配置
            lineConfig = currentFontConfig[currentFontConfig.length - 1];
          }
        } else if (globalFontConfig && Array.isArray(globalFontConfig)) {
          // 如果没有分镜配置但有全局配置，使用全局配置
          if (lineIndex < globalFontConfig.length) {
            lineConfig = globalFontConfig[lineIndex];
          } else if (globalFontConfig.length > 0) {
            // 如果行数超过配置数量，使用最后一个配置
            lineConfig = globalFontConfig[globalFontConfig.length - 1];
          }
        }
        ...
        if (lineConfig) {
          ...
          // 使用内联样式覆盖
          styledLine += `{\\fn${fontStyle.fontname}\\fs${fontStyle.fontsize}\\c${fontStyle.primaryColour}\\b${fontStyle.bold}}`;
        }
      ...
      if (sectionTitleAnimation) {
        switch (sectionTitleAnimation) {
          ...
          case "particle_shatter":
            // 粒子破碎消失（真正的破碎效果）
            animationEffect = `{\\fad(200,400)\\t(0,150,\\fscx110\\fscy110\\1a&H00&\\3a&H00&\\blur1)\\t(150,250,\\fscx95\\fscy105\\1a&H20&\\3a&H20&\\blur2)\\t(250,350,\\fscx80\\fscy120\\1a&H40&\\3a&H40&\\blur3)\\t(350,450,\\fscx60\\fscy140\\1a&H60&\\3a&H60&\\blur4)\\t(450,550,\\fscx40\\fscy160\\1a&H80&\\3a&H80&\\blur5)\\t(550,650,\\fscx20\\fscy180\\1a&HA0&\\3a&HA0&\\blur6)\\t(650,750,\\fscx10\\fscy200\\1a&HC0&\\3a&HC0&\\blur7)\\t(750,850,\\fscx5\\fscy220\\1a&HE0&\\3a&HE0&\\blur8)\\t(850,950,\\fscx0\\fscy0\\1a&HFF&\\3a&HFF&)}`;
            break;
          case "particle_dissolve":
            // 粒子溶解消失（逐渐消散）- 增强溶解效果
            animationEffect = `{\\fad(300,600)\\t(0,200,\\1a&H10&\\3a&H10&\\blur1\\fscx102\\fscy98)\\t(200,400,\\1a&H30&\\3a&H30&\\blur2\\fscx98\\fscy102)\\t(400,600,\\1a&H50&\\3a&H50&\\blur3\\fscx95\\fscy105)\\t(600,800,\\1a&H70&\\3a&H70&\\blur4\\fscx90\\fscy110)\\t(800,1000,\\1a&H90&\\3a&H90&\\blur5\\fscx85\\fscy115)\\t(1000,1200,\\1a&HB0&\\3a&HB0&\\blur6\\fscx80\\fscy120)\\t(1200,1400,\\1a&HFF&\\3a&HFF&\\fscx0\\fscy0)}`;
            break;
          case "particle_explosion":
            // 粒子爆炸消失（爆炸飞散）- 强化爆炸效果
            animationEffect = `{\\fad(150,300)\\t(0,100,\\fscx95\\fscy95\\1a&H10&\\3a&H10&\\blur1)\\t(100,150,\\fscx80\\fscy80\\1a&H00&\\3a&H00&\\blur2)\\t(150,200,\\fscx200\\fscy200\\1a&H00&\\3a&H00&\\1c&HFFFFFF&\\blur8)\\t(200,300,\\fscx150\\fscy150\\1a&H30&\\3a&H30&\\blur6)\\t(300,450,\\fscx80\\fscy80\\1a&H60&\\3a&H60&\\blur4)\\t(450,600,\\fscx40\\fscy40\\1a&H90&\\3a&H90&\\blur2)\\t(600,750,\\fscx10\\fscy10\\1a&HC0&\\3a&HC0&)\\t(750,900,\\1a&HFF&\\3a&HFF&\\fscx0\\fscy0)}`;
            break;
          case "particle_dust":
            // 粒子尘化消失（化为尘埃）- 增强尘化效果
            animationEffect = `{\\fad(250,500)\\t(0,200,\\1a&H20&\\3a&H20&\\1c&HFFFFFF&\\blur1\\fscx101\\fscy99)\\t(200,400,\\1a&H40&\\3a&H40&\\1c&HE0E0E0&\\blur2\\fscx99\\fscy101)\\t(400,600,\\1a&H60&\\3a&H60&\\1c&HC0C0C0&\\blur3\\fscx97\\fscy103)\\t(600,800,\\1a&H80&\\3a&H80&\\1c&H808080&\\blur4\\fscx95\\fscy105)\\t(800,1000,\\1a&HA0&\\3a&HA0&\\blur5\\fscx90\\fscy110)\\t(1000,1200,\\1a&HC0&\\3a&HC0&\\blur6\\fscx80\\fscy120)\\t(1200,1400,\\1a&HFF&\\3a&HFF&\\fscx0\\fscy0)}`;
            break;
          case "particle_sparkle":
            // 粒子闪烁消失（星光消散）- 增强闪烁效果
            animationEffect = `{\\fad(200,400)\\t(0,150,\\1a&H00&\\3a&H00&\\1c&HFFFFFF&\\3c&HFFFF00&\\fscx150\\fscy150\\blur3\\bord4)\\t(150,300,\\1a&H10&\\3a&H10&\\1c&HFFFF80&\\3c&HFFFF80&\\fscx130\\fscy130\\blur2\\bord3)\\t(300,450,\\1a&H30&\\3a&H30&\\1c&HFFFF40&\\fscx110\\fscy110\\blur1\\bord2)\\t(450,600,\\1a&H50&\\3a&H50&\\fscx90\\fscy90\\blur2)\\t(600,750,\\1a&H70&\\3a&H70&\\fscx70\\fscy70\\blur3)\\t(750,900,\\1a&H90&\\3a&H90&\\fscx50\\fscy50\\blur4)\\t(900,1050,\\1a&HB0&\\3a&HB0&\\fscx30\\fscy30\\blur5)\\t(1050,1200,\\1a&HFF&\\3a&HFF&\\fscx0\\fscy0)}`;
            break;
    ...
    if (topTitle && duration > 0) {
      ...
      // 使用动画类生成全局标题ASS内容
      // 为全局标题创建专用的 wrapCJK 函数，使用 titleLineBreak
      const wrapCJKForTitle = (s) => wrapCJK(s, titleLineBreak);
    ...
    if (endTitle) {
      ...
      console.log(
        `[3/5] 添加结尾标题: "${endTitle}" (时长: ${finalEndTitleDuration}秒)`
      );
      ...
      // 结尾标题在视频播放完成后显示，延长视频总时长
      const endTitleStart = duration; // 从原视频结束时开始
      const endTitleEnd = duration + finalEndTitleDuration; // 延长视频时长
      ...
      if (
        ...
        try {
          ...
          if (typewriterSoundPath) {
            console.log(`🎵 结尾标题打字机音效已生成: ${typewriterSoundPath}`);
          }
        } catch (error) {
          console.warn(`⚠️ 生成打字机音效失败: ${error.message}`);
        }
      }

      // 使用动画类生成结尾标题ASS内容
      // 为结尾标题创建专用的 wrapCJK 函数，使用 titleLineBreak
      const wrapCJKForEndTitle = (s) => wrapCJK(s, titleLineBreak);
      ...
      // 调试日志：检查结尾标题ASS内容
      console.log(`[DEBUG] 结尾标题ASS内容长度: ${endTitleASS.length} 字符`);
      console.log(
        `[DEBUG] 结尾标题时间范围: ${endTitleStart}s - ${endTitleEnd}s`
      );

      // 存储打字机音效信息
      if (typewriterSoundPath) {
        // 将音效信息存储在全局变量中，供后续合成使用
    ...
    if (disclaimerText) {
      ...
      try {
        ...
        console.log(
          `[警告] 底部免责声明处理失败: ${error.message}，继续处理其他字幕`
        );
      }
    }
    ...
    // 处理婴儿哭声（如果启用）
    if (useBabyCry) {
      console.log(`[3/5] 准备添加婴儿哭声音效`);

      const babyCryPath = path.join(process.cwd(), "sounds", "婴儿哭声.mp3");
      try {
        ...
        console.log(`🍼 婴儿哭声已准备: ${babyCryOutputPath} (2秒)`);
      } catch (error) {
        console.warn(`⚠️ 婴儿哭声文件不存在: ${babyCryPath}`);
      }
    }
    ...
    if (voiceList && voiceList.length > 0) {
      ...
      try {
        const processedVoices = await processVoiceClips(
          voiceList,
          outputDir,
          duration
        );
        ...
        if (processedVoices.length > 0) {
          console.log(
            `📁 处理完成，共生成 ${processedVoices.length} 个配音文件`
          );

          // 存储配音信息
          global.voiceListInfo = processedVoices;
        }
      } catch (voiceError) {
        console.warn(`⚠️ 配音处理失败: ${voiceError.message}`);
        global.voiceListInfo = [];
      }
    }
    ...
    // 如果有结尾标题，需要在字幕烧录阶段就延长视频
    const finalEndTitleDuration = endTitle ? endTitleDuration || 2 : 0;
    const totalVideoDuration = duration + finalEndTitleDuration;

    let burnCmd;
    if (finalEndTitleDuration > 0) {
      // 有结尾标题：延长视频并烧录字幕
      console.log(
        `[3/5] 检测到结尾标题，延长视频时长: ${duration}s -> ${totalVideoDuration}s`
      );
      burnCmd = `ffmpeg -y -i "${videoPath}" -vf "tpad=stop_mode=clone:stop_duration=${finalEndTitleDuration},subtitles=filename='${assPathEscaped}'" -t ${totalVideoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_COPY} "${tempVideoPath}"`;
    } else {
      // 无结尾标题：正常烧录字幕
      burnCmd = `ffmpeg -y -i "${videoPath}" -vf "subtitles=filename='${assPathEscaped}'" ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_COPY} "${tempVideoPath}"`;
    }
...
async function compositeWithMusic(
  ...
  if (useBgMusic) {
    ...
    const originalVideoDuration = parseFloat(
      info.format?.duration ||
        info.streams.find((s) => s.codec_type === "video")?.duration ||
        0
    );
    ...
    // 计算包含结尾标题的总视频时长
    const finalEndTitleDuration = endTitle ? endTitleDuration || 2 : 0;
    const videoDuration = originalVideoDuration + finalEndTitleDuration;
    ...
    console.log(
      `[4/5] 原视频时长: ${originalVideoDuration}s，总时长(含结尾标题): ${videoDuration}s`
    );

    // 创建视频滤镜：视频已在字幕烧录阶段延长，这里只需要复制
    ...
    if (typewriterSound && typewriterSound.path) {
      try {
        ...
        console.log(`[4/5] 检测到打字机音效: ${typewriterSound.path}`);
        console.log(
          `[4/5] 打字机音效时间: ${typewriterSound.start}s - ${typewriterSound.start + typewriterSound.duration}s (延迟${typewriterSound.start * 1000}ms)`
        );
      } catch (error) {
        console.warn(`⚠️ 打字机音效文件不存在: ${typewriterSound.path}`);
      }
    }
    ...
    if (babyCryInfo && babyCryInfo.path) {
      try {
        ...
        hasBabyCry = true;
        console.log(`[4/5] 检测到婴儿哭声: ${babyCryInfo.path}`);
        console.log(`[4/5] 婴儿哭声时间: 0s - ${babyCryInfo.duration}s`);
      } catch (error) {
        console.warn(`⚠️ 婴儿哭声文件不存在: ${babyCryInfo.path}`);
      }
    }

    if (hasOriginalAudio) {
      const audioEffects = [];
      if (hasTypewriterSound) audioEffects.push("打字机音效");
      if (hasBabyCry) audioEffects.push("婴儿哭声");
      const effectsText =
        audioEffects.length > 0 ? `、${audioEffects.join("、")}` : "";
      console.log(
        `[4/5] 检测到原视频有音频，将混合原音频、背景音乐${effectsText}`
      );

      if (hasTypewriterSound && hasBabyCry) {
        // 混合原视频音频、背景音乐、打字机音效和婴儿哭声（4轨混合）
        console.log(
          `🎵 打字机音效时间: ${typewriterSound.start}s - ${typewriterSound.start + typewriterSound.duration}s (延迟${typewriterSound.start * 1000}ms)`
        );
        console.log(
          `[4/5] 正在合并音频轨道（原音频+背景音乐+打字机音效+婴儿哭声）...`
        );
        execCommand(
          `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" -i "${typewriterSound.path}" -i "${babyCryInfo.path}" -filter_complex "${videoFilter};[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_WITH_BG}[a0];[1:a]volume=${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME},apad=pad_dur=${videoDuration}[a1];[2:a]adelay=${typewriterSound.start * 1000}|${typewriterSound.start * 1000},volume=${CONFIG_AUDIO_VOLUME.TYPEWRITER_SOUND_VOLUME}[a2];[3:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=2[a3];[a0][a1][a2][a3]amix=inputs=4:duration=longest:normalize=0[aout]" -map "[v]" -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
          true // 显示进度
        );
        ...
        console.log(`[4/5] 正在合并音频轨道（原音频+背景音乐+打字机音效）...`);
        execCommand(
          `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" -i "${typewriterSound.path}" -filter_complex "${videoFilter};[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_WITH_BG}[a0];[1:a]volume=${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME},apad=pad_dur=${videoDuration}[a1];[2:a]adelay=${typewriterSound.start * 1000}|${typewriterSound.start * 1000},volume=${CONFIG_AUDIO_VOLUME.TYPEWRITER_SOUND_VOLUME}[a2];[a0][a1][a2]amix=inputs=3:duration=longest:normalize=0[aout]" -map "[v]" -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
          true // 显示进度
        );
      } else if (hasBabyCry) {
        // 混合原视频音频、背景音乐和婴儿哭声
        console.log(`[4/5] 正在合并音频轨道（原音频+背景音乐+婴儿哭声）...`);
        execCommand(
          `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" -i "${babyCryInfo.path}" -filter_complex "${videoFilter};[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_WITH_BG}[a0];[1:a]volume=${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME},apad=pad_dur=${videoDuration}[a1];[2:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=2[a2];[a0][a1][a2]amix=inputs=3:duration=longest:normalize=0[aout]" -map "[v]" -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
          true // 显示进度
        );
      } else {
        // 检查是否有片段配音
        const processedVoices = global.voiceListInfo || [];

        if (processedVoices.length > 0) {
          // 混合原视频音频、背景音乐和片段配音
          console.log(`[4/5] 正在合并音频轨道（原音频+背景音乐+配音）...`);
          console.log(`🎵 音频轨道数量: ${2 + processedVoices.length}`);

          // 构建输入文件列表
          const inputs = [`"${videoPath}"`, `"${resolvedBgMusicPath}"`];
          ...
          // 构建filter_complex
          let filterParts = [
            `[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_WITH_BG}[a0]`, // 原视频音频
            `[1:a]volume=${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME},apad=pad_dur=${videoDuration}[a1]`, // 背景音乐
          ];

          // 添加配音轨道
          processedVoices.forEach((voice, index) => {
            const inputIndex = index + 2;
            const delay = voice.startTime * 1000; // 转换为毫秒
            filterParts.push(
              `[${inputIndex}:a]adelay=${delay}|${delay},volume=${CONFIG_AUDIO_VOLUME.VOICE_CLIP_VOLUME},apad=pad_dur=${videoDuration}[a${inputIndex}]`
            );
          });

          // 构建amix - 禁用自动归一化以保持音量
          const audioInputs = [`[a0]`, `[a1]`].concat(
            processedVoices.map((_, index) => `[a${index + 2}]`)
          );
          const totalInputs = audioInputs.length;
          filterParts.push(
            `${audioInputs.join("")}amix=inputs=${totalInputs}:duration=longest:normalize=0[aout]`
          );

          const filterComplex = `${videoFilter};${filterParts.join(";")}`;

          execCommand(
            `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" ${processedVoices.map((voice) => `-i "${voice.file}"`).join(" ")} -filter_complex "${filterComplex}" -map "[v]" -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        } else {
          // 混合原视频音频和背景音乐
          console.log(`[4/5] 正在合并音频轨道（原音频+背景音乐）...`);
          execCommand(
            `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" -filter_complex "${videoFilter};[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_WITH_BG}[a0];[1:a]volume=${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME},apad=pad_dur=${videoDuration}[a1];[a0][a1]amix=inputs=2:duration=longest:normalize=0[aout]" -map "[v]" -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        }
      }
      ...
      const effectsText =
        audioEffects.length > 0 ? `、${audioEffects.join("、")}` : "";
      console.log(`[4/5] 原视频无音频，使用背景音乐${effectsText}`);
      ...
      if (hasTypewriterSound && hasBabyCry) {
        ...
        console.log(
          `[4/5] 正在合并音频轨道（背景音乐+打字机音效+婴儿哭声）...`
        );
        execCommand(
          `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" -i "${typewriterSound.path}" -i "${babyCryInfo.path}" -filter_complex "${videoFilter};[1:a]volume=${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME},apad=pad_dur=${videoDuration}[a1];[2:a]adelay=${typewriterSound.start * 1000}|${typewriterSound.start * 1000},volume=${CONFIG_AUDIO_VOLUME.TYPEWRITER_SOUND_VOLUME}[a2];[3:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=2[a3];[a1][a2][a3]amix=inputs=3:duration=longest:normalize=0[aout]" -map "[v]" -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
          true // 显示进度
        );
      } else if (hasTypewriterSound) {
        // 混合背景音乐和打字机音效
        ...
        console.log(`[4/5] 正在合并音频轨道（背景音乐+打字机音效）...`);
        execCommand(
          `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" -i "${typewriterSound.path}" -filter_complex "${videoFilter};[1:a]volume=${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME},apad=pad_dur=${videoDuration}[a1];[2:a]adelay=${typewriterSound.start * 1000}|${typewriterSound.start * 1000},volume=${CONFIG_AUDIO_VOLUME.TYPEWRITER_SOUND_VOLUME}[a2];[a1][a2]amix=inputs=2:duration=longest:normalize=0[aout]" -map "[v]" -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
          true // 显示进度
        );
      } else if (hasBabyCry) {
        // 混合背景音乐和婴儿哭声
        console.log(`[4/5] 正在合并音频轨道（背景音乐+婴儿哭声）...`);
        execCommand(
          `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" -i "${babyCryInfo.path}" -filter_complex "${videoFilter};[1:a]volume=${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME},apad=pad_dur=${videoDuration}[a1];[2:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=2[a2];[a1][a2]amix=inputs=2:duration=longest:normalize=0[aout]" -map "[v]" -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
          true // 显示进度
        );
      } else {
        // 检查是否有片段配音
        ...
        if (processedVoices.length > 0) {
          // 原视频无音频，混合背景音乐和片段配音
          console.log(`[4/5] 正在合并音频轨道（背景音乐+配音）...`);
          console.log(`🎵 音频轨道数量: ${1 + processedVoices.length}`);
          ...
          console.log(
            `🎵 背景音乐音量: ${CONFIG_AUDIO_VOLUME.BACKGROUND_MUSIC_VOLUME}, 配音音量: ${CONFIG_AUDIO_VOLUME.VOICE_CLIP_VOLUME}`
          );
          console.log(`🔄 背景音乐循环播放从0秒开始，时长: ${videoDuration}s`);
          ...
          console.log(`[4/5] 正在添加背景音乐...`);
          execCommand(
            `ffmpeg -y -i "${videoPath}" -stream_loop -1 -i "${resolvedBgMusicPath}" -filter_complex "${videoFilter}" -map "[v]" -map 1:a:0 -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        }
      }
    ...
    if (hasTypewriterSound || hasBabyCry) {
      ...
      if (hasTypewriterSound) audioEffects.push("打字机音效");
      if (hasBabyCry) audioEffects.push("婴儿哭声");
      console.log(`[4/5] 不使用背景音乐，但添加${audioEffects.join("、")}`);
      ...
      if (hasOriginalAudio) {
        if (hasTypewriterSound && hasBabyCry) {
          // 原视频有音频，混合原音频、打字机音效和婴儿哭声
          execCommand(
            `ffmpeg -y -i "${videoPath}" -i "${typewriterSound.path}" -i "${babyCryInfo.path}" -filter_complex "${videoFilter};[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_SOLO}[a0];[1:a]adelay=${typewriterSound.start * 1000}|${typewriterSound.start * 1000},volume=${CONFIG_AUDIO_VOLUME.TYPEWRITER_SOUND_VOLUME}[a1];[2:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=2[a2];[a0][a1][a2]amix=inputs=3:duration=longest:normalize=0[aout]" -map "[v]" -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        } else if (hasTypewriterSound) {
          // 原视频有音频，混合原音频和打字机音效
          execCommand(
            `ffmpeg -y -i "${videoPath}" -i "${typewriterSound.path}" -filter_complex "${videoFilter};[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_SOLO}[a0];[1:a]adelay=${typewriterSound.start * 1000}|${typewriterSound.start * 1000},volume=${CONFIG_AUDIO_VOLUME.TYPEWRITER_SOUND_VOLUME}[a1];[a0][a1]amix=inputs=2:duration=longest:normalize=0[aout]" -map "[v]" -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        } else if (hasBabyCry) {
          // 原视频有音频，混合原音频和婴儿哭声
          execCommand(
            `ffmpeg -y -i "${videoPath}" -i "${babyCryInfo.path}" -filter_complex "${videoFilter};[0:a]volume=${CONFIG_AUDIO_VOLUME.ORIGINAL_AUDIO_VOLUME_SOLO}[a0];[1:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=2[a1];[a0][a1]amix=inputs=2:duration=longest:normalize=0[aout]" -map "[v]" -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        }
      } else {
        if (hasTypewriterSound && hasBabyCry) {
          // 原视频无音频，添加打字机音效和婴儿哭声
          execCommand(
            `ffmpeg -y -i "${videoPath}" -i "${typewriterSound.path}" -i "${babyCryInfo.path}" -filter_complex "${videoFilter};[1:a]adelay=${typewriterSound.start * 1000}|${typewriterSound.start * 1000},volume=${CONFIG_AUDIO_VOLUME.TYPEWRITER_SOUND_VOLUME}[a1];[2:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=2[a2];[a1][a2]amix=inputs=2:duration=longest:normalize=0,apad=pad_dur=${videoDuration}[aout]" -map "[v]" -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        } else if (hasTypewriterSound) {
          // 原视频无音频，只添加打字机音效
          execCommand(
            `ffmpeg -y -i "${videoPath}" -i "${typewriterSound.path}" -filter_complex "${videoFilter};[1:a]adelay=${typewriterSound.start * 1000}|${typewriterSound.start * 1000},volume=${CONFIG_AUDIO_VOLUME.TYPEWRITER_SOUND_VOLUME},apad=pad_dur=${videoDuration}[aout]" -map "[v]" -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        } else if (hasBabyCry) {
          // 原视频无音频，只添加婴儿哭声
          execCommand(
            `ffmpeg -y -i "${videoPath}" -i "${babyCryInfo.path}" -filter_complex "${videoFilter};[1:a]volume=${CONFIG_AUDIO_VOLUME.BABY_CRY_VOLUME},apad=pad_dur=${videoDuration}[aout]" -map "[v]" -map "[aout]" -t ${videoDuration} ${encodingParams} -c:a ${CONFIG_VIDEO.AUDIO_CODEC_AAC} -b:a ${CONFIG_VIDEO.AUDIO_BITRATE} "${finalVideoPath}"`,
            true // 显示进度
          );
        }
      }
...
/**
 * 生成封面图片，在青年图片上添加人物名称
 * @param {string} projectName - 项目名称（用于查找青年图片）
 * @param {string} title - 标题文本（用于提取人物名称）
 * @param {string} outputDir - 输出目录
 * @returns {string} - 生成的封面图片路径
 */
async function generateCover(projectName, title, outputDir) {
  console.log(`[封面生成] 开始生成封面图片...`);
  ...
  if (!(await fs.pathExists(youthImagePath))) {
    console.warn(`⚠️ 青年图片不存在: ${youthImagePath}`);
    return null;
  }
  ...
  if (title) {
    ...
    if (!personName) {
      ...
      if (lines.length > 0) {
        ...
        // 提取最后一行中的中文字符（通常是人名）
        const nameMatch = lastLine.match(/([^\s，,。！？；：的]+)/);
        if (nameMatch) {
  ...
  if (!personName) {
    console.warn(`⚠️ 无法从标题中提取人物名称: ${title}`);
    personName = "历史人物"; // 默认名称
  }

  console.log(`[封面生成] 提取到人物名称: ${personName}`);
  ...
  try {
    ...
    console.log(
      `[封面生成] 图片尺寸: ${imageWidth}x${imageHeight}, 文字位置Y: ${textY}`
    );

    // 优先尝试使用 ImageMagick 添加文字，如果失败则使用 FFmpeg
    let success = false;

    // 方法1：尝试使用 ImageMagick (magick 命令)
    try {
      // 首先尝试使用平台特定字体的中文字符
      ...
      if (Math.abs(newSize - originalSize) < 100) {
        // 文件大小几乎没变，可能字体不存在，尝试不指定字体
        console.warn(`⚠️ 指定字体可能不存在，尝试使用默认字体`);
        ...
        if (Math.abs(newSize2 - originalSize) < 100) {
          // 如果还是失败，尝试英文名称作为最后手段
          console.warn(`⚠️ 中文字符渲染失败，使用英文名称作为备选`);
          const englishName =
            personName === "李政道"
              ? "Li Zhengdao"
              : personName === "邓稼先"
          ...
          execCommand(magickCommand);
          console.log(`✅ 使用英文名称 "${englishName}" 生成封面成功`);
        } else {
          console.log(`✅ 使用默认字体的中文名称 "${personName}" 生成封面成功`);
        }
      } else {
        console.log(
          `✅ 使用指定字体 "${platformFont}" 的中文名称 "${personName}" 生成封面成功`
        );
      }

      success = true;
    } catch (magickError) {
      console.warn(`⚠️ ImageMagick 失败: ${magickError.message}`);

      // 方法2：尝试使用 FFmpeg drawtext
      try {
        ...
        success = true;
        console.log(`✅ 使用 FFmpeg drawtext 生成封面成功`);
      } catch (ffmpegError) {
        console.warn(`⚠️ FFmpeg drawtext 失败: ${ffmpegError.message}`);

        // 方法3：尝试使用默认字体的 FFmpeg
        try {
          ...
          success = true;
          console.log(`✅ 使用 FFmpeg 默认字体生成封面成功`);
        } catch (defaultError) {
          console.warn(`⚠️ FFmpeg 默认字体也失败: ${defaultError.message}`);
        }
      }
    }

    if (!success) {
      throw new Error("所有文字叠加方法都失败了");
    }

    console.log(`✅ 封面图片生成完成: ${coverPath}`);
    return coverPath;
  } catch (error) {
    ...
    try {
      ...
      console.log(`📋 已复制青年图片作为封面: ${coverPath}`);
      return coverPath;
    } catch (copyError) {
      console.error(`❌ 复制青年图片失败: ${copyError.message}`);
      return null;
    }
  }
}
...
export default async function runHistoryPerson(config) {
  ...
  let {
    ...
    skipTempCleanup,
    useBabyCry,
    disclaimerText = "AIGC生成 无真人肖像 只为致敬", // 默认免责声明文字
    sectionFirstTitleTime, // 第一个字幕的特殊显示时长
    sectionTitleInterval, // 分镜字幕间隔时间（秒）
    sectionLastTitleOffset, // 最后一个分镜提前显示时间（秒）
    voiceList, // 片段配音列表
    useLines = false, // 是否使用 lines 目录下的配音文件
    freezeFrame, // 定格帧时长（秒），如 0.2 表示每5秒最后一帧定格0.2秒
    freezeFrameAnimation, // 定格帧动画效果
    titleLineBreak = "\n", // history-person 标题换行符，默认为 \n
  } = config;
  ...
  console.log(
    `第一个字幕时长: ${sectionFirstTitleTime ? `${sectionFirstTitleTime}秒` : "默认(4.8秒)"}`
  );
  console.log(
    `分镜字幕间隔: ${sectionTitleInterval ? `${sectionTitleInterval}秒` : "默认(6秒)"}`
  );
  console.log(
    `最后分镜提前: ${sectionLastTitleOffset ? `${sectionLastTitleOffset}秒` : "默认(2秒)"}`
  );
  ...
  console.log(
    `速度优化: ${enableSpeedOptimization ? "启用(多线程+预设优化)" : "禁用"}`
  );
  console.log(`临时文件清理: ${skipTempCleanup ? "跳过(节省时间)" : "启用"}`);
  console.log(`使用 lines 配音: ${useLines ? "启用" : "禁用"}`);
  console.log(
    `定格帧效果: ${freezeFrame ? `启用 (每5秒定格${freezeFrame}秒)` : "禁用"}`
  );
  if (freezeFrame && freezeFrame > 0) {
    const animation =
      freezeFrameAnimation || CONFIG_FREEZE_FRAME_ANIMATION.DEFAULT;
    console.log(`定格帧动画: ${animation}`);
  }
  ...
  // 如果启用 useLines，自动生成 voiceList
  if (useLines && name) {
    console.log(`\n🎵 检测到 useLines=true，正在自动生成配音列表...`);
    try {
      const linesDir = path.join(process.cwd(), "output", name, "lines");

      // 检查 lines 目录是否存在
      if (await fs.pathExists(linesDir)) {
        ...
        const mp3Files = files
          ...
          .map((file) => {
            ...
            return match ? { name: file, index: parseInt(match[1]) } : null;
          })
        ...
        if (mp3Files.length > 0) {
          // 获取每个配音文件的实际时长
          const voicesWithDuration = [];
          for (const item of mp3Files) {
            const voiceFilePath = path.join(linesDir, item.name);
            try {
              const probe = execCommand(
                `ffprobe -v quiet -print_format json -show_format "${voiceFilePath}"`
              );
              const info = JSON.parse(probe.stdout);
              const duration = parseFloat(info.format?.duration || 0);
              voicesWithDuration.push({
                ...item,
                duration: duration,
              });
            } catch (error) {
              console.warn(`⚠️ 无法获取 ${item.name} 的时长，使用默认5秒`);
              voicesWithDuration.push({
                ...item,
                duration: 5,
              });
          ...
          // 计算配音时间，考虑定格帧和配音实际时长
          const baseFreezeFrame = freezeFrame || 0;
          let currentTime = 0;
          const dynamicFreezeFrames = []; // 记录每个片段的实际定格时长

          voiceList = voicesWithDuration.map((item, index) => {
            const startTime = currentTime;
            const voiceDuration = item.duration;
            const normalPlayDuration = 5; // 正常播放时长
            const baseFreezeDuration = baseFreezeFrame; // 基础定格时长

            let actualFreezeDuration = baseFreezeDuration;
            let endTime;

            if (baseFreezeFrame > 0) {
              // 有定格帧模式
              const totalAvailableTime =
                normalPlayDuration + baseFreezeDuration;

              if (voiceDuration > totalAvailableTime) {
                // 配音时长超过可用时间，需要延长定格帧
                actualFreezeDuration = voiceDuration - normalPlayDuration;
                endTime = startTime + voiceDuration;
                console.log(
                  `   ⏱️ 配音${index} 时长${voiceDuration.toFixed(2)}s > 可用${totalAvailableTime.toFixed(2)}s，延长定格帧到 ${actualFreezeDuration.toFixed(2)}s`
                );
              } else {
                // 配音时长在可用时间内，使用基础定格时长
                endTime = startTime + normalPlayDuration + baseFreezeDuration;
              }

              dynamicFreezeFrames.push(actualFreezeDuration);
              currentTime = endTime;
            } else {
              // 无定格帧模式
              endTime = startTime + Math.max(normalPlayDuration, voiceDuration);
              currentTime = endTime;
            }

            return {
              duration: [startTime, endTime],
              voice: `${name}/lines/${item.name.replace(".mp3", "")}`,
              voiceDuration: voiceDuration, // 保存实际配音时长
              freezeDuration: actualFreezeDuration, // 保存实际定格时长
            };
          });

          // 保存动态定格帧信息，供定格帧处理使用
          if (baseFreezeFrame > 0) {
            global.dynamicFreezeFrames = dynamicFreezeFrames;
            console.log(
              `✅ 成功生成 ${voiceList.length} 个配音片段 (动态定格帧):`
            );
          } else {
            console.log(`✅ 成功生成 ${voiceList.length} 个配音片段:`);
          }

          voiceList.forEach((voice, index) => {
            const freezeInfo =
              voice.freezeDuration > 0
                ? ` (配音${voice.voiceDuration.toFixed(2)}s, 定格${voice.freezeDuration.toFixed(2)}s)`
                : "";
            console.log(
              `   ${index}: ${voice.duration[0].toFixed(1)}s-${voice.duration[1].toFixed(1)}s -> ${voice.voice}${freezeInfo}`
            );
          });

          // 保存配音信息供字幕使用
          global.voiceListInfo = voiceList.map((voice) => ({
            startTime: voice.duration[0],
            endTime: voice.duration[1],
            voiceDuration: voice.voiceDuration,
            freezeDuration: voice.freezeDuration,
          }));
        } else {
          console.warn(`⚠️ ${linesDir} 目录下没有找到 mp3 文件`);
        }
      } else {
        console.warn(`⚠️ lines 目录不存在: ${linesDir}`);
      }
    } catch (error) {
      console.warn(`⚠️ 自动生成 voiceList 失败: ${error.message}`);
    }
  }
  ...
  if (hasManualSectionTitle) {
    ...
    try {
      ...
      if (await fs.pathExists(processedDataPath)) {
        console.log(
          `\n🔍 发现 ${name} 的processed_data.json，正在生成sectionTitle和字体配置...`
        );
        ...
        if (processedData.segments && processedData.segments.length > 0) {
          ...
          for (let i = 0; i < processedData.segments.length; i++) {
            ...
            sectionTitle.push(segment.title);

            console.log(`📝 生成第${i + 1}个分镜: ${segment.title}`);
          }
          ...
          // 提取字体配置
          if (processedData.globalFontConfig) {
            config.globalFontConfig = processedData.globalFontConfig;
            console.log(
              `📝 提取到全局字体配置: ${processedData.globalFontConfig.length}个配置`
            );
          }

          if (processedData.titleFontConfig) {
            config.titleFontConfig = processedData.titleFontConfig;
            console.log(
              `📝 提取到全局标题字体配置: ${processedData.titleFontConfig.length}个配置`
            );
          }

          if (processedData.segments) {
            config.segments = processedData.segments;
            const segmentsWithFontConfig = processedData.segments.filter(
              (s) => s.fontConfig
            );
            if (segmentsWithFontConfig.length > 0) {
              console.log(
                `📝 提取到分镜字体配置: ${segmentsWithFontConfig.length}个分镜有字体配置`
              );
            }
          }
  ...
  try {
    ...
    // 2. 标准化视频分辨率为720x1280
    console.log(
      `[2/5] 正在标准化视频分辨率为 ${CONFIG_VIDEO.DEFAULT_RESOLUTION}...`
    );
    let standardizedPath = await standardizeAspect(videoPath, outputDir, {
      targetAspect: CONFIG_VIDEO.DEFAULT_ASPECT_RATIO,
      targetResolution: CONFIG_VIDEO.DEFAULT_RESOLUTION,
      fitMode: CONFIG_VIDEO.DEFAULT_FIT_MODE,
    ...
    // 2.5. 添加定格帧效果（如果配置了 freezeFrame）
    if (freezeFrame && freezeFrame > 0) {
      const animation =
        freezeFrameAnimation || CONFIG_FREEZE_FRAME_ANIMATION.DEFAULT;

      // 调试：检查 dynamicFreezeFrames 是否存在
      const dynamicFreezeFrames = global.dynamicFreezeFrames || [];
      console.log(
        `[DEBUG] dynamicFreezeFrames 数量: ${dynamicFreezeFrames.length}`
      );
      if (dynamicFreezeFrames.length > 0) {
        console.log(
          `[DEBUG] 前3个定格时长: ${dynamicFreezeFrames
            .slice(0, 3)
            .map((d) => d.toFixed(2))
            .join(", ")}`
        );
      ...
      standardizedPath = await addFreezeFrames(
        standardizedPath,
        outputDir,
        freezeFrame,
        5, // 每5秒一个定格点
      ...
      // 保存定格帧时长信息，供后续字幕计算使用
      global.freezeFrameDuration = freezeFrame;
    } else {
      global.freezeFrameDuration = 0;
    }
    ...
    const videoWithTitles = await addSectionTitles(
      ...
      disclaimerText,
      sectionFirstTitleTime,
      sectionTitleInterval, // 传入分镜字幕间隔时间
      sectionLastTitleOffset, // 传入最后一个分镜提前显示时间
      voiceList, // 片段配音列表
      useLines, // 新增：是否使用 lines 模式
      titleLineBreak, // history-person 标题换行符
      config.globalFontConfig, // 全局字体配置
      config.segments, // 分镜数据（包含字体配置）
      config.titleFontConfig // 全局标题字体配置
    );
    ...
    if (name && title) {
      try {
        ...
        if (coverPath) {
          console.log(`📸 封面图片已生成: ${coverPath}`);
        }
      } catch (error) {
        console.warn(`[警告] 封面生成失败: ${error.message}`);
      }
    }
    ...
    if (!enableSpeedOptimization && totalTime > 60) {
      console.log(
        `💡 性能提示: 处理时间较长，建议启用 enableSpeedOptimization: true`
      );
    }
    if (!skipTempCleanup && totalTime > 30) {
      console.log(
        `💡 性能提示: 可启用 skipTempCleanup: true 跳过临时文件清理以节省时间`
      );
    }
    if (qualityMode === "high" && totalTime > 120) {
      console.log(
        `💡 性能提示: 可使用 qualityMode: "fast" 或 "turbo" 以提升处理速度`
      );
    }
  } catch (error) {
