...
export class TitleAnimation {
  ...
  generateGlobalTitleASS(
    ...
    if (isEndTitle) {
      ...
      // 计算整体居中的起始位置
      const availableWidth = videoWidth - CONFIG_SPACING.GLOBAL_TITLE_MARGIN_LEFT - CONFIG_SPACING.GLOBAL_TITLE_MARGIN_RIGHT;
      
      if (maxLineWidth > availableWidth) {
        // 标题过长，在可用区域内居中
        endTitleStartX = CONFIG_SPACING.GLOBAL_TITLE_MARGIN_LEFT + Math.floor((availableWidth - maxLineWidth) / 2);
        console.log(`[结尾标题] 标题过长 (${maxLineWidth}px > ${availableWidth}px)，在可用区域内居中: ${endTitleStartX}px`);
      } else {
        // 标题适中，在整个屏幕宽度内居中
        ...
        console.log(`[结尾标题] 标题适中，居中显示: ${endTitleStartX}px (中心: ${centerX}px)`);
      }
    }
    ...
    for (let lineIndex = 0; lineIndex < topTitleLines.length; lineIndex++) {
      ...
      if (isEndTitle) {
        ...
        if (lineWidth > availableWidth) {
          // 标题过长，在可用区域内居中
          const centerInAvailable = CONFIG_SPACING.GLOBAL_TITLE_MARGIN_LEFT + Math.floor((availableWidth - lineWidth) / 2);
          startX = centerInAvailable;
          console.log(`[标题] 标题过长 (${lineWidth}px > ${availableWidth}px)，在可用区域内居中: ${startX}px`);
        } else {
          // 标题适中，在整个屏幕宽度内居中
          ...
          console.log(`[标题] 标题适中，居中显示: ${startX}px (中心: ${centerX}px)`);
        }
      }
