...
export class TitleAnimation {
  ...
  generateGlobalTitleASS(
    ...
    const topTitleY = isEndTitle 
      ...
      : Math.floor(videoHeight * CONFIG_SPACING.GLOBAL_TITLE_POSITION_Y_PERCENT); // 普通标题在顶部
    ...
    const startY = Math.max(
      ...
      topTitleY - Math.floor(totalTitleHeight / 2)
    );
    ...
    if (isEndTitle) {
      const endTitleCharSpacing = topTitleFontSize * CONFIG_SPACING.GLOBAL_TITLE_CHAR_SPACING_FACTOR * 1.3; // 增加30%字符间距
      let maxLineWidth = 0;
      
      // 计算最大行宽度
      for (let i = 0; i < topTitleLines.length; i++) {
        const lineParsedChars = this.parseKeywords(topTitleLines[i]);
        const lineDisplayChars = lineParsedChars.map((pc) => pc.char);
        const lineWidth = lineDisplayChars.length * endTitleCharSpacing;
      ...
      const maxX = videoWidth - CONFIG_SPACING.GLOBAL_TITLE_MARGIN_RIGHT - maxLineWidth;
    ...
    for (let lineIndex = 0; lineIndex < topTitleLines.length; lineIndex++) {
      ...
      let charSpacing, startX;
      if (isEndTitle) {
        // 结尾标题：使用更大的字符间距，整体居中但每行左对齐
        charSpacing = topTitleFontSize * CONFIG_SPACING.GLOBAL_TITLE_CHAR_SPACING_FACTOR * 1.3; // 增加30%字符间距
        startX = endTitleStartX; // 所有行使用相同的左起始位置
      } else {
        // 普通标题：使用正常字符间距，每行单独居中
        charSpacing = topTitleFontSize * CONFIG_SPACING.GLOBAL_TITLE_CHAR_SPACING_FACTOR;
