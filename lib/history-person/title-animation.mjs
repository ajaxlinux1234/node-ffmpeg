...
export class TitleAnimation {
  ...
  calculateEnglishCharSpacing(text, baseFontSize, charIndex, chars) {
    ...
    // 优先检查：连续数字不添加字符间距
    if (/[0-9]/.test(currentChar) && nextChar && /[0-9]/.test(nextChar)) {
      return 0; // 数字之间不添加间距
    }

    // 检查常见缩写词（如NBA, FIFA等），如果是则不添加字符间距
    const commonAbbreviations = ['NBA', 'NFL', 'MLB', 'NHL', 'FIFA', 'UEFA', 'IOC', 'USA', 'UK', 'EU', 'UN', 'WHO', 'NASA', 'FBI', 'CIA', 'CEO', 'CTO', 'CFO', 'COO', 'HR', 'IT', 'AI', 'VR', 'AR', 'GPS', 'USB', 'WiFi', 'CPU', 'GPU', 'RAM', 'SSD', 'HDD', 'TV', 'DVD', 'CD', 'MP3', 'MP4', 'PDF', 'JPG', 'PNG', 'HTML', 'CSS', 'JS', 'API', 'URL', 'HTTP', 'HTTPS', 'BMW', 'IBM', 'AMD', 'ATM', 'LED', 'LCD', 'OLED'];
    
    // 检查当前位置是否是缩写词的一部分
    for (const abbr of commonAbbreviations) {
      const abbrIndex = text.indexOf(abbr);
      if (abbrIndex !== -1 && charIndex >= abbrIndex && charIndex < abbrIndex + abbr.length - 1) {
        // 当前字符是缩写词的一部分，不添加字符间距
        return 0;
      }
    }
