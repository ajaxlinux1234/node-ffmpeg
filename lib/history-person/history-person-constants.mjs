...
export const CONFIG_SUBTITLE = {
  ...
  SUBTITLE_POSITION_Y_PERCENT: 0.3, // 字幕Y位置百分比（从下往上）
  ...
  SECTION_TITLE_INTERVAL: 6.0, // 分镜字幕间隔时间（秒）- 可在config.mjs中覆盖
  SECTION_LAST_TITLE_OFFSET: 6.0, // 最后一个分镜提前显示时间（秒）- 在视频结束前多少秒开始显示
};
...
export const CONFIG_TITLE_ANIMATION = {
  ...
  ANIMATION_TYPES: {
    ...
    // 粒子破碎系列
    PARTICLE_SHATTER: "particle_shatter", // 粒子破碎消失（碎片飞散）
    PARTICLE_DISSOLVE: "particle_dissolve", // 粒子溶解消失（逐渐消散）
    PARTICLE_EXPLOSION: "particle_explosion", // 粒子爆炸消失（爆炸飞散）
    PARTICLE_DUST: "particle_dust", // 粒子尘化消失（化为尘埃）
    PARTICLE_SPARKLE: "particle_sparkle", // 粒子闪烁消失（星光消散）
  },
