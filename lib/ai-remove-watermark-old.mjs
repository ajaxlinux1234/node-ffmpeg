...
async function buildDefaultMask({ width, height }, workDir, maskOpt = {}) {
  ...
  try {
    ...
    execSync(`magick -size ${width}x${height} xc:black -fill white -draw "rectangle ${x},${y} ${x + mw},${y + mh}" "${maskPath}"`, { stdio: 'inherit' });
    return maskPath;
  } catch {}
  try {
    execSync(`convert -size ${width}x${height} xc:black -fill white -draw "rectangle ${x},${y} ${x + mw},${y + mh}" "${maskPath}"`, { stdio: 'inherit' });
    return maskPath;
  } catch {
    execSync(`ffmpeg -y -f lavfi -i color=c=black:s=${width}x${height} -vf drawbox=${x}:${y}:${mw}:${mh}:white@1:fill -frames:v 1 "${maskPath}"`, { stdio: 'inherit' });
    return maskPath;
  }
}
...
async function ensureCvEnv() {
  ...
  execSync(`"${pythonBin}" -m pip install --upgrade pip wheel`, { stdio: 'inherit' });
  // Minimal deps for OpenCV inpainting
  execSync(`"${pipBin}" install --upgrade "setuptools<70" packaging`, { stdio: 'inherit' });
  execSync(`"${pipBin}" install opencv-python-headless pillow`, { stdio: 'inherit' });

  return { pythonBin };
}

// Create and run a small python script that applies OpenCV inpainting to each frame using a provided mask
async function runCvInpaint(
  framesDir,
  outDir,
  maskPath,
  autoDetect = null,
...
def build_badge_mask(img):
    # Specialized for small bright rounded rectangle badge at top-left ("AI生成")
    h, w = img.shape[:2]
    rx0, ry0, rx1, ry1 = 0, 0, int(w*0.40), int(h*0.22)
    roi = img[ry0:ry1, rx0:rx1]
    gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
    # enhance edges and uniform bright areas
    blur = cv2.GaussianBlur(gray, (3,3), 0)
    _, th = cv2.threshold(blur, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    # focus on very bright
    _, bright = cv2.threshold(blur, 210, 255, cv2.THRESH_BINARY)
    cand = cv2.bitwise_or(th, bright)
    ...
    for c in contours:
        ...
        if area < 60:
            continue
        # rounded badge has medium aspect ratio and high fill ratio
        ar = wc/float(hc+1e-6)
        if 1.3 <= ar <= 4.5 and 10 <= wc <= int(0.35*w) and 10 <= hc <= int(0.18*h):
            # fill ratio inside box
            box = cand[y:y+hc, x:x+wc]
            fill = box.mean()/255.0
            if fill > 0.35:
                x0 = rx0 + max(0, x - 3)
                y0 = ry0 + max(0, y - 3)
                x1 = rx0 + min(rx1-rx0, x + wc + 3)
                y1 = ry0 + min(ry1-ry0, y + hc + 3)
                mask[y0:y1, x0:x1] = 255
    if mask.sum() > 0:
        mask = cv2.dilate(mask, np.ones((max(2,dilate_px),max(2,dilate_px)), np.uint8), iterations=1)
    return mask
...
if auto_mode and files:
    ...
    for idx in indices:
        ...
        if auto_mode == 'full-text':
            ...
            # try general corner first, then badge specialization
            m = build_auto_mask(img, auto_mode)
            if m.sum() < 200:  # very small -> try badge
                m = cv2.bitwise_or(m, build_badge_mask(img))
...
for i, f in enumerate(files, 1):
    ...
    if mask.sum() == 0 and auto_mode:
        ...
        if auto_mode == 'full-text':
            mask = build_full_text_mask(img)
        else:
            mask = build_auto_mask(img, auto_mode)
            if mask.sum() < 200:
                mask = cv2.bitwise_or(mask, build_badge_mask(img))
    # hard fallback if still tiny: put a conservative rectangle at top-left
    if mask.sum() < 150 and auto_mode in ('top-left', 'full-text'):
        hbox = int(0.12*h)
        wbox = int(0.22*w)
        mask[5:5+hbox, 5:5+wbox] = 255
    _, mask_bin = cv2.threshold(mask, 127, 255, cv2.THRESH_BINARY)
    if dilate_px > 0:
        k = max(1, dilate_px)
        mask_bin = cv2.dilate(mask_bin, np.ones((k,k), np.uint8), iterations=1)
    if dbg:
        # overlay preview for some frames (first and middle)
        if i == 1 or i == (len(files)//2):
            overlay = img.copy()
            cnts, _ = cv2.findContours(mask_bin, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            cv2.drawContours(overlay, cnts, -1, (0,0,255), 2)
            name = 'mask_preview_overlay.png' if i == 1 else 'mask_preview_overlay_mid.png'
  ...
  execSync(`"${pythonBin}" "${scriptPath}" "${framesDir}" "${outDir}" "${maskPath}" "${autoArg}" ${inpaintRadius} ${debug ? "1" : "0"} ${dilatePx} ${extraExpandPx} '${extraRegionsJson}'`, { stdio: 'inherit' });
}
...
async function rebuildVideo(originalPath, inpaintedDir, meta) {
  ...
  if (meta.pix_fmt) {
    args.push("-pix_fmt", meta.pix_fmt);
  }
  if (meta.color_primaries) {
    args.push("-color_primaries", meta.color_primaries);
  }
  if (meta.color_transfer) {
    args.push("-color_trc", meta.color_transfer);
  }
  if (meta.color_space) {
    args.push("-colorspace", meta.color_space);
  }
  args.push(...extra);
  args.push("-c:a", "copy");
  args.push(outPath);
...
async function processVideo(url, options = {}) {
  ...
  console.log(`[2/2] 处理完成! 输出文件: ${outVideo}`);
