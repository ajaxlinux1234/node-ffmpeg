...
export async function runMergeOptions(config, options = {}) {
  ...
  try {
    ...
    if (processedData.videoReplaceUrls && Array.isArray(processedData.videoReplaceUrls)) {
      console.log(`\n🔄 检测到 videoReplaceUrls，准备替换视频片段...`);
      console.log(`   - 原始视频数量: ${processedData.urls.length}`);
      console.log(`   - 替换片段数量: ${processedData.videoReplaceUrls.length}`);
      ...
      for (let i = 0; i < processedData.videoReplaceUrls.length; i++) {
        ...
        if (!url || !timeRange || !Array.isArray(timeRange) || timeRange.length !== 2) {
        ...
        console.log(`   ✅ 将替换合并后视频的 ${timeRange[0]}s-${timeRange[1]}s 片段`);
      }
    ...
    await updateConfigFile(configPath, (content) => {
      ...
      if (videoReplaceConfig) {
        const replaceConfigStr = JSON.stringify(videoReplaceConfig, null, 6).replace(
          /\n/g,
          "\n      "
        );
      ...
      // 添加 switch 配置（保持原有的转场效果）
      const switchMatch = content.match(/"merge-video":\s*\{[\s\S]*?switch:\s*"([^"]*)"/);
    ...
    await updateConfigFile(
      ...
      (content) => {
        ...
        if (processedData.segments && Array.isArray(processedData.segments)) {
          const sectionTitles = processedData.segments.map(segment => segment.title);
          ...
          if (sectionTitleRegex.test(content)) {
            ...
            console.log(`✅ 已更新 sectionTitle (${sectionTitles.length} 个标题)`);
          }
        }
...
使用方法:
  ...
  npx node-ffmpeg-tools merge-options --name "项目名称"   # 更新 jimeng-video-config.mjs 的 name
  npx node-ffmpeg-tools merge-options --num 11           # 更新所有 historyNum 为 11
  npx node-ffmpeg-tools merge-options --name "项目名称" --num 11  # 同时更新两者
  npx node-ffmpeg-tools merge-options --help             # 显示帮助信息

命令行选项:
  --name, -n <项目名称>    更新 jimeng-video-config.mjs 中的 name 常量
...
注意事项:
  ...
  • --name 参数会更新 lib/auto-deepseek-jimeng/jimeng-video-config.mjs
