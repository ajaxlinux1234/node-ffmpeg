...
export async function runMergeOptions(config, options = {}) {
  ...
  try {
    ...
    try {
      ...
      // 兼容性处理：支持 url 和 urls 两种字段名
      if (processedData.url && !processedData.urls) {
        processedData.urls = processedData.url;
        console.log(`ℹ️ 检测到 'url' 字段，自动转换为 'urls' 字段`);
      }
      ...
      console.log(
        `   - 背景音乐: ${processedData.musicName ? `${processedData.musicName}.mp3` : "使用配置文件中的设置"}`
      );
    } catch (error) {
      throw new Error(
    ...
    if (
      ...
      for (let i = 0; i < processedData.videoReplaceUrls.length; i++) {
        ...
        // 添加更详细的错误检查
        if (!replaceItem) {
          console.warn(`   ⚠️  替换项 ${i + 1} 为空，跳过`);
          continue;
        }

        const { url, timeRange, index } = replaceItem;

        // 检查必需字段
        if (!url) {
          console.warn(`   ⚠️  替换项 ${i + 1} 缺少 url 字段，跳过`);
          continue;
        }

        if (!timeRange) {
          console.warn(`   ⚠️  替换项 ${i + 1} 缺少 timeRange 字段，跳过`);
          continue;
        }

        if (!Array.isArray(timeRange)) {
          console.warn(`   ⚠️  替换项 ${i + 1} 的 timeRange 不是数组，跳过`);
          continue;
        }

        if (timeRange.length !== 2) {
          console.warn(`   ⚠️  替换项 ${i + 1} 的 timeRange 长度不为2，跳过`);
          continue;
        }
        ...
        // 检查 index 字段（如果存在）
        // index 应该基于合并后视频的片段数量，而不是原始视频数量
        const maxSegmentIndex = processedData.segments
          ? processedData.segments.length - 1
          : processedData.urls.length - 1;
        if (
          typeof index !== "undefined" &&
          (index < 0 || index > maxSegmentIndex)
        ) {
          console.warn(
            `   ⚠️  替换项 ${i + 1} 的 index ${index} 超出片段范围 (0-${maxSegmentIndex})，跳过`
          );
          continue;
        }
        ...
        console.log(
          `   ✅ 将替换${typeof index !== "undefined" ? `片段 ${index} 的` : "合并后视频的"} ${timeRange[0]}s-${timeRange[1]}s 片段`
        );
      }
    ...
    // 更新 url、title、sectionTitle 和 musicName
    await updateConfigFile(
      ...
      (content) => {
        ...
        // 更新 musicName - 从 processed_data.json 中读取并更新 bg-music 路径
        if (processedData.musicName) {
          const musicPath = `music/${processedData.musicName}.mp3`;
          console.log(`🎵 检测到 musicName: ${processedData.musicName}`);
          console.log(`🎵 更新 bg-music 路径为: ${musicPath}`);

          // 先找到 history-person 配置块，然后在其中替换 bg-music
          const historyPersonStart = content.indexOf('"history-person": {');
          if (historyPersonStart !== -1) {
            // 找到 history-person 配置块的结束位置
            let braceCount = 0;
            let historyPersonEnd = historyPersonStart;
            let inString = false;
            let escapeNext = false;

            for (let i = historyPersonStart; i < content.length; i++) {
            ...
            // 提取 history-person 配置块
            const historyPersonBlock = content.substring(
              historyPersonStart,
              historyPersonEnd
            );

            // 在 history-person 配置块中查找并替换 bg-music
            const bgMusicRegex = /("bg-music":\s*")[^"]*(")/;
            if (bgMusicRegex.test(historyPersonBlock)) {
              const updatedBlock = historyPersonBlock.replace(
                bgMusicRegex,
                `$1${musicPath}$2`
              );
              content =
                content.substring(0, historyPersonStart) +
                updatedBlock +
                content.substring(historyPersonEnd);
              console.log(
                `✅ 已更新 history-person 中的 bg-music: ${musicPath}`
              );
            } else {
              console.warn(`⚠️ 未找到 history-person 配置中的 bg-music 字段`);
            }
          } else {
            console.warn(`⚠️ 未找到 history-person 配置块`);
          }
        } else {
          console.log(
            `ℹ️ processed_data.json 中未找到 musicName 字段，保持原有 bg-music 配置`
          );
        }
...
功能说明:
  ...
  4. 更新 history-person 配置（包括 sectionTitle 和 bg-music）并生成最终视频
...
{
  "name": "项目名称",
  "urls": ["video1.mp4", "video2.mp4", "video3.mp4"],  // 或使用 "url" 字段（兼容性支持）
  "musicName": "国际歌_start5.2s_clip",  // 可选：背景音乐名称，会自动设置为 music/{musicName}.mp3
  "videoReplaceUrls": [  // 可选：替换指定视频片段
    {
      "index": 1,  // 替换第2个片段（索引从0开始，基于segments数组长度）
...
注意事项:
  ...
  • processed_data.json 必须包含 urls（或 url）和 title 字段
  • 支持 "urls" 和 "url" 两种字段名（自动兼容转换）
  • 如果 processed_data.json 包含 musicName 字段，会自动更新 history-person 的 bg-music 为 music/{musicName}.mp3
  • 流程会自动更新 config.mjs 文件（包括 sectionTitle 和 bg-music）
